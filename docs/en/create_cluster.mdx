...
Ensure that all nodes in the <Term name="product" textCase="capitalize" />'s `global` cluster can access the DCS platform:
- **Port 7443**: Required for DCS platform API access
- **Port 8443**: Required for DCS platform web console access

:::warning
**Important Connectivity Requirement**
All nodes in the ACP global cluster must have network connectivity to both ports (7443 and 8443) on the DCS platform. Failure to ensure connectivity will prevent cluster creation and management operations.
:::

### 4. DCS Platform Credential Requirements

The DCS platform credentials used for cluster creation must meet the following requirements:

**Required User Configuration**

The user must be configured in the DCS platform with:

- **User Type**: `Interface interconnection user` (接口互联用户)
- **Role**: `administrator`

**Password Policy Requirement**

In the DCS platform, navigate to **System Management** → **Rights Management** → **Rights Management Policy** and verify the following setting:

- **Policy Name**: `Whether to modify the password of an interface interconnection user upon password resetting and first login` (接口互联用户在重置密码和首次登录时是否修改密码)
- **Value**: Must be set to `No`

:::warning
**Configuration Impact**

If the password policy is set to `Yes`:
- The interface interconnection user's password will be forced to change upon first login or password reset
- This will break the authentication between the Cluster API provider and the DCS platform
- Cluster creation and management operations will fail

Ensure the policy is set to `No` before configuring the authentication Secret in the "Configure DCS Authentication" section.
:::
...
**Datastore Cross-Host Access Requirement**

The datastore clusters configured in `datastoreClusterName` must support cross-host access across all physical machines in the DCS platform. This ensures that VMs can be created successfully regardless of which physical machine is selected by the DCS platform.

If the datastore does not support cross-host access:
- VMs may be created successfully on one physical machine (e.g., Machine A)
- VM creation may fail when the DCS platform selects a different physical machine (e.g., Machine B)
- This can lead to unpredictable failures during cluster provisioning or scaling

:::warning
**Shared Storage for Ignition**

If the datastore used for system volume and ignition does not support file upload operations, you must provide a shared storage solution that supports multi-host mounting (e.g., NFS). This ensures that ignition configuration files can be accessed by VMs regardless of which physical machine they are created on.
:::
