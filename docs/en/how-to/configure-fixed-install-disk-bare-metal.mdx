...
Create a `SeedImage` that references this MachineRegistration, or rebuild the existing SeedImage after changing the install device. That `SeedImage` must also carry the `state` partition sizing described in [Size COS_STATE before the first install](../create-cluster/bare-metal.mdx#size-cos-state) — the Elemental default of 8 GiB is too small for later reprovisioning, and the size cannot be changed without reinstalling the host.
