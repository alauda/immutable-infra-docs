...
This pattern is the failure mode of the AIT-70656 hostname/FQDN bug that is fixed in `cluster-api-provider-hcs` v1.0.1+. The buggy controller renders cloud-init with the full FQDN string as `hostname` and enables `prefer_fqdn_over_hostname: true`; the resulting node has a POSIX hostname containing dots, which kubeadm init does not handle, so kube-apiserver never starts.
...
Path A (recommended) — finish upgrading the `cluster-api-provider-hcs` plugin to v1.0.1+, then trigger a rolling replacement of the affected control plane and worker machines so the new cloud-init runs on freshly booted VMs. The procedure is documented in [Plugin Behavior Upgrades](/upgrade-cluster/huawei-cloud-stack#plugin-behavior-upgrades) on the HCS upgrade page.

Path B (manual mitigation per node) is described in the same upgrade section. It does not survive `transactional-update -c reboot` on immutable OSes (MicroOS / SLE Micro), so it is acceptable only as a one-off intervention on a single node where rolling replacement is genuinely blocked.

### Prevent

Re-running the cluster create with the fixed plugin already installed avoids the pattern entirely. New manifests for HCS should follow [Hostname behavior on the node](/create-cluster/huawei-cloud-stack#hostname-behavior-on-the-node) on the cluster create page when picking dotted versus short hostnames in `HCSMachineConfigPool`.
