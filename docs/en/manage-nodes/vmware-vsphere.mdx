...
- A slot whose `status.configStatuses[].state` is `InUse` or `Released` cannot be removed, cannot have its `network.primary.ip` or `.ipv6` changed, and cannot lose a persistent disk or change one's `sizeGiB`, `mountPath`, or assigned `unitNumber`. Adding a slot, or adding a disk to an existing slot, is accepted.
