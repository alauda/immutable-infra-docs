...
- Verify that each `VSphereResourcePool` is referenced by only one `KubeadmControlPlane` or one `MachineDeployment`. Sharing a pool across multiple groups causes machines to stall at `WaitingForBootstrapData`.
