...
- You know the installed provider version. The behavior on this page is validated against VMware vSphere Provider `v1.0.17`. See [Provider Requirements](../create-cluster/vmware-vsphere.mdx#provider-requirements).

:::warning
**A Day-2 `apply` can be rejected**

The rollout and pool rules are enforced on `UPDATE` as well as `CREATE`, so they are checked on every edit this page makes:

- Re-applying an archived `MachineDeployment` manifest re-submits the whole object. If that file omits `strategy.rollingUpdate.maxSurge: 0` or a `maxUnavailable` of `1` or more, the apply is rejected — including an apply whose only intended change is `replicas`. Correct the file before you apply it. `strategy.type: OnDelete` is exempt from both rules.
- A slot whose `status.configStatuses[].state` is `InUse` or `Released` cannot be removed, cannot have its `network.primary.ip` or `.ipv6` changed, and cannot lose a disk from `persistentDisks[]` or `ephemeralDisks[]` or change one's `sizeGiB`, `mountPath`, or assigned `unitNumber`. Adding a slot, or adding a disk to either list on an existing slot, is accepted.

Match a rejection message against [Admission rejections](../create-cluster/vmware-vsphere.mdx#admission-rejections).
:::
...
The example uses the disk layout of a cluster created on provider `v1.0.17`. A pool created on an earlier provider has no `ephemeralDisks[]` and declares `/var/cpaas`, `/var/lib/containerd`, and `/var/lib/etcd` in `persistentDisks[]`; match the slots that pool already has instead of introducing `ephemeralDisks[]` into it.
...
The second command replaces the whole `MachineDeployment` spec, not only `replicas`. Confirm the archived file carries `strategy.rollingUpdate.maxSurge: 0` and a `maxUnavailable` of `1` or more before you apply it, or the scale-out is rejected.
...
Persistent disks are owned by `VSphereMachineConfigPool` slots rather than `VSphereMachineTemplate`, so update the pool slots before you start the replacement. That change can only be additive: the slot you are about to replace a node on is `InUse` or `Released`, which accepts a new disk but rejects removing one or changing an existing disk's `sizeGiB`, `mountPath`, or assigned `unitNumber`. A different geometry for an existing disk therefore needs a new slot, not an edit of the current one. See [Allocated slots are immutable](../create-cluster/vmware-vsphere.mdx#create-the-machine-config-pools).
...
## Runtime Topology Changes \{#runtime-topology-changes}
...
1. Add the required slot definitions to the existing `VSphereMachineConfigPool` before increasing replicas or starting replacement. An allocated slot accepts additions only; see the admission warning in [Prerequisites](#prerequisites).
...
- Do not remove the required `var-cpaas` persistent disk, or the required `var-lib-kubelet` and `var-lib-containerd` ephemeral disks, or the control-plane `var-lib-etcd` ephemeral disk.
- Keep a new slot's disk lists identical to the slots already in that pool. `ephemeralDisks[]` exists from provider `v1.0.17`, but a pool created on an earlier provider declares `/var/cpaas`, `/var/lib/containerd`, and `/var/lib/etcd` in `persistentDisks[]`; do not convert those slots, and do not mix layouts inside one pool.
- Append, never reshape. A slot that is `InUse` or `Released` accepts new entries in `persistentDisks[]` and `ephemeralDisks[]`, which take effect on the next VM recreation, but it rejects removing a disk or changing its `sizeGiB`, `mountPath`, or `unitNumber`. Resizing means planning a new slot.
