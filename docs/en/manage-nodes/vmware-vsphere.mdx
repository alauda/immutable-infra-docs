...
The example uses the disk layout of a cluster created on provider `v1.0.17`. A pool created on an earlier provider declares the same disks in `persistentDisks[]`; match the slots that pool already has instead of introducing `ephemeralDisks[]` into it.
...
- Do not remove the required `var-cpaas` persistent disk, or the required `var-lib-containerd` and control-plane `var-lib-etcd` ephemeral disks.
- Keep a new slot's disk lists identical to the slots already in that pool. `ephemeralDisks[]` exists from provider `v1.0.17`, but a pool created on an earlier provider declares every required disk in `persistentDisks[]`; do not convert those slots, and do not mix layouts inside one pool.
