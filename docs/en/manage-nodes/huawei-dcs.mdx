...
When a node template declares a dedicated data disk at `/var/lib/containerd`, the DCS provider's bootstrap configuration stores pod logs under `/var/lib/containerd/pods` and links `/var/log/pods` to that path. Keep the disk mounted at `/var/lib/containerd` when replacing the node if you need the pod-log data to remain on the containerd data disk.
...
Create a `DCSIpHostnamePool` named `<cluster-name>-worker-pool`:
...
Create a `DCSMachineTemplate` named `<cluster-name>-worker-template`:
