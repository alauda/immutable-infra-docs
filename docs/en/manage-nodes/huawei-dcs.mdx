...
- The control plane must be deployed before performing node operations. See [Create Cluster](../create-cluster/huawei-dcs.mdx) for setup instructions.
...
2. **Extend IP Pool**
   ...
   :::info
   **IP Pool Expansion**
   The IP pool must contain at least as many entries as the desired replica count. Add new IP entries for each additional worker node you plan to deploy.
   :::
...
3. **Scale Down the MachineDeployment**
    ...
    :::info
    **Replica Count Must Match Annotated Machines**
    Reduce replicas by exactly the number of annotated machines.
...
| Max Surge | number | No | Default: 0, must be ≥ 0 |
...
Kubernetes upgrades follow a specific sequence: Control Plane Node Pool must be upgraded first, followed by Worker Node Pools.

**Version requirement**: This workflow requires Fleet Essentials and Alauda Container Platform DCS Infrastructure Provider `1.0.13` or later. If the provider version is earlier than `1.0.13`, use the YAML-based upgrade workflow in this document.

:::warning
**Upgrade Prerequisites**
- Distribution Version upgrade must be completed first (see [Upgrading Distribution Version](../../upgrade-cluster/index.mdx))
- Control Plane Node Pool must be in Running state
- Sufficient IP Pool capacity for rolling updates
:::

### Checking Available Upgrades

In the cluster details **Node Pools** Tab:
- Node Pools with available upgrades show **Upgrade available** indicator
- Click on the Node Pool card to view Current vs Target versions
...
**Navigation**: Node Pools Tab → Locate Control Plane Node Pool → Click **Upgrade**

**Upgrade Dialog**:
- **Current Version**: Displays the current Kubernetes version
- **Target Version**: Automatically selected (latest minor version supported)

Click **Confirm** to start the upgrade.

**Monitoring**:
- Watch the Node Pool status
- Nodes roll update one by one (maxSurge=0, maxUnavailable=1)
...
| Pool State | Upgrade Available |
|------------|-------------------|
| Control Plane not started | ❌ Upgrade the Control Plane Node Pool first |
| Control Plane upgrading | ❌ Wait for the Control Plane Node Pool upgrade to complete |
| Control Plane upgraded | ✅ Upgrade button enabled |
| Pool not in Running state | ❌ Upgrade is unavailable when the Worker Node Pool is not in the Running state |
...
- [Create Clusters](../create-cluster/huawei-dcs.mdx)
