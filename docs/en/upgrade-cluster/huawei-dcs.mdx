...
The authoritative mapping between an ACP release, its MicroOS image, the Kubernetes version, and the matching CoreDNS and etcd image tags lives in [OS Support Matrix](../overview/os-support-matrix.mdx). Locate the row that corresponds to the target ACP version before you start; the row supplies every value the YAML steps below need.

The four cells you read from that row map to the upgrade manifests as follows:
...
The CoreDNS and etcd image tags are control-plane-only because `clusterConfiguration` is a `KubeadmControlPlane` field. Worker nodes inherit container image versions from the new VM template; the `MachineDeployment` does not carry its own `dns`/`etcd` tags.
...
Before you start, collect the four required values from the target ACP row in the OS Support Matrix as described in [Required Values From the OS Support Matrix](#required-values).
...
2. **Patch the `KubeadmControlPlane` with all four target values**
