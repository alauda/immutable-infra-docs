...
| Bare Metal | `v1.0.0` and later | None; `v1.0.0` is the first supported release for this procedure. |
...
If the target OS Support Matrix row uses Kube-OVN chart `v4.4` or later, upgrade the infrastructure provider to the full-source reconciliation version in the table before continuing. A manual Kube-OVN source patch is not a substitute for that provider upgrade because the same provider releases also reconcile the required CoreDNS and kube-proxy repository changes. DCS Self-built VIP is a separate creation-time endpoint feature and requires DCS Provider `v1.0.22+` with ACP `v4.4+`; changing an existing external endpoint to internal is not part of this upgrade procedure.
...
This procedure applies to Bare Metal provider `v1.0.0` and later. There is no earlier supported release flow to select.
...
- **DCS `v1.0.22+`, HCS `v1.0.4+`, vSphere `v1.0.16+`, and Bare Metal `v1.0.0+`**: restore the previous `cpaas.io/kube-ovn-version` annotation. The provider restores the matching chart name, revision, and complete specification. Do not patch the source manually.
...
|----------|-------------|
| [Huawei DCS](./huawei-dcs.mdx) | Huawei Datacenter Virtualization Solution |
| [Huawei Cloud Stack](./huawei-cloud-stack.mdx) | Huawei Cloud Stack |
| [VMware vSphere](./vmware-vsphere.mdx) | VMware vSphere virtualization platform |
| [Bare Metal](./bare-metal.mdx) | Bare-metal servers without virtualization |
