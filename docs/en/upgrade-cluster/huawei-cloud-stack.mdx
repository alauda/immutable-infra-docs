...
- The VM image created from the Alauda OS image published for the target ACP release is present in the HCS environment. The upgrade fails if the image is not present when the new `HCSMachineTemplate` is applied.
...
The authoritative mapping between an ACP release, the Kubernetes version, and the matching CoreDNS, etcd, and Kube-OVN versions lives in [OS Support Matrix](../overview/os-support-matrix.mdx). Locate the row that corresponds to the target ACP version before you start; the row supplies the component values the procedure below needs.

The ECS image name is not in the matrix. It is the name the image was uploaded under in HCS from the Alauda OS image published for the target ACP release — see [Downloading Alauda OS](../overview/os-support-matrix.mdx#downloading-alauda-os) — and it is what `HCSMachineTemplate.spec.template.spec.imageName` must be set to on both the control plane and worker templates.
...
1. **Create a new `HCSMachineTemplate` for the target Kubernetes version**
   ...
   - Set `metadata.name` to `<new-template-name>`.
   - Set `spec.template.spec.imageName` to the name of the target Alauda OS image as uploaded to HCS.
