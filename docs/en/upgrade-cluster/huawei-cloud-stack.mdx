...
| **B — Node-level manual fix** | SSH into each existing node and edit `/etc/hostname` and `/etc/hosts` by hand. | The kubelet's reported node name changes only after a kubelet restart; existing `Node` objects keep the old name and need to be reconciled or the node re-registered. ACP load balancer routing and monitoring labels can drift. The change does not persist across a `transactional-update -c reboot` if it lands on an immutable subvolume. | Not recommended for production. Acceptable only as a one-off intervention on a single node where rolling replacement is genuinely blocked. |
| **C — DaemonSet that mutates the host** | Run a privileged DaemonSet that mounts `/host` and rewrites `/etc/hostname` and `/etc/hosts` on every node. | Same risks as path B, multiplied by the number of nodes the DaemonSet covers. | Discouraged. Treat as a stopgap and follow up with path A. |

For immutable OSes (MicroOS / SLE Micro), only path A is durable across `transactional-update -c reboot` — paths B and C touch the running subvolume which may be reverted by the next transactional reboot.
