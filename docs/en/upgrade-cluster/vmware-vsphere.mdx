...
| Pool-managed ephemeral disks | `VSphereMachineConfigPool.spec.configs[].ephemeralDisks[]` | ❌ Never — the VMDK is deleted with the VM and a new empty disk is created for the replacement | Rebuildable node-local data that should not sit on the system disk, including the standard `/var/lib/kubelet`, `/var/lib/containerd`, and `/var/lib/etcd` disks. |
