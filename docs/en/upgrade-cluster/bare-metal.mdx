...
| `elemental upgrade` fails with `no space left on device` | The `state` partition is too small. Check `df -h /` on the host: an 8 GiB `COS_STATE` holds the running system and its snapshots comfortably while they all share extents with one image, but an upgrade to a different image has to hold two unrelated systems at once and does not fit. Each failed attempt also leaves a partial snapshot behind under `/.snapshots/`, so free space keeps shrinking. Recover by deleting the partial snapshot directories that are **not** listed by `btrfs subvolume list /` and are **not** the running snapshot (`findmnt -no FSROOT /`), then let the plan retry. Hosts installed before [COS_STATE sizing](../create-cluster/bare-metal.mdx#size-cos-state) was applied need a reinstall to grow the partition. |
