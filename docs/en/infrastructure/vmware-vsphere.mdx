...
queries:
  ...
  - vsphere self-built vip prerequisites
  - vsphere persistent disk sizing and unit number
---

# VMware vSphere Infrastructure Preparation

This document helps you prepare the infrastructure and collect the values required to create a VMware vSphere workload cluster. Complete this checklist before you apply the manifests in [Creating Clusters on VMware vSphere](../create-cluster/vmware-vsphere.mdx). The provider behavior and fields on this page are validated against VMware vSphere Provider `v1.0.17`.
...
4. If an optional feature is not enabled, omit the corresponding YAML block exactly as described in [Creation-Time Topology Variants](../create-cluster/vmware-vsphere.mdx#creation-time-topology-variants).
...
| Product-derived | Kubernetes, Alauda OS image, Kube-OVN chart, CoreDNS, etcd, provider chart, and CPI versions | Select the exact set published for the target ACP release. Use the [OS Support Matrix](../overview/os-support-matrix.mdx) for component versions and the OS download page for the OS image, together with provider release notes and delivered package metadata; do not choose the newest Registry tag independently. |
...
This section covers the external LoadBalancer endpoint, which the baseline cluster uses. Provision it before cluster creation and follow [Plan the Control Plane Endpoint](../how-to/control-plane-endpoint.mdx). A cluster can instead be created with the provider-managed Self-built VIP, which needs no external LoadBalancer and requires ACP `v4.4` or later; see [Control Plane Endpoint Modes](../create-cluster/vmware-vsphere.mdx#control-plane-endpoint-modes).

| Parameter | Placeholder | Required | Validation or Notes | Example | Actual Value |
|-----------|-------------|----------|---------------------|---------|--------------|
| Control plane VIP or FQDN | `<vip>` | Yes | The frontend address is allocated and stable. An FQDN is accepted for an external LoadBalancer only, and requires DNS and API server certificate SAN planning; a Self-built VIP must be IPv4. | `10.10.10.10` | - |
...
| Virtual router ID | `<vrid>` | When the cluster uses the Self-built VIP | Keepalived `virtual_router_id`, `1`–`255`, unique in the control-plane Layer-2 domain. | `51` | - |
| VIP interface | `<vip_interface>` | When the cluster uses the Self-built VIP | Host interface that carries the VIP, at most 15 characters. Leave empty to let the provider match the interface holding the node's primary IP; set it explicitly on multi-NIC nodes, because it cannot be changed later. | `ens192` | - |
...
| Kubernetes version | `<k8s_version>` | Yes | Use the version required by the target platform release. The baseline manifests omit `imagePullCredentialsVerificationPolicy` for Kubernetes 1.34 or earlier. For Kubernetes 1.35 or later, add `imagePullCredentialsVerificationPolicy: NeverVerify` to the kubelet patch in both the control-plane and worker manifests. | `v1.35.6-acp.1` (needs the Kubernetes 1.35 kubelet patch) | - |
| Control plane replicas | `<cp_replicas>` | Yes | Must be `3` or more. The provider's admission webhook rejects a pool-backed `KubeadmControlPlane` with fewer replicas, and a `replicas: 1` control plane cannot be upgraded later. See [Provider Requirements](../create-cluster/vmware-vsphere.mdx#provider-requirements). | `3` | - |
...
| `kube-ovn` version | `<kube_ovn_version>` | Yes | Must match the platform network plugin requirements. | `v4.4.1` | - |
| `kube-ovn-join-cidr` | `<kube_ovn_join_cidr>` | Yes | Must not overlap with other networks. | `100.64.0.0/16` | - |
| CoreDNS image tag | `<dns_image_tag>` | Yes | Use the tag published for the target ACP release, copied verbatim with its `-v<acp>` suffix: at ACP `v4.4` and later, the tag staged under `acp/coredns`; below `v4.4`, the one under `tkestack/coredns`. The provider reads the suffix to select that repository, so a tag without it resolves to `tkestack/coredns` and fails to pull on ACP `v4.4` or later. Leave `dns.imageRepository` unset; see [Provider-managed fields on your manifests](../create-cluster/vmware-vsphere.mdx#provider-managed-fields). | `1.14.3-v4.4.6` | - |
| etcd image tag | `<etcd_image_tag>` | Yes | Use the tag approved for the Kubernetes version. Take it from the same [OS Support Matrix](../overview/os-support-matrix.mdx) row as the other version values. | `v3.6.13-acp.0` | - |
...
The authoritative chart-name boundary is in [Upgrade Kube-OVN Before the Control Plane](../upgrade-cluster/index.mdx#upgrade-kube-ovn-before-the-control-plane).
...
### Validate the locally preloaded images \{#validate-the-locally-preloaded-images}
...
`VSphereMachineTemplate.spec.template.spec.diskGiB` and the two disk lists under `VSphereMachineConfigPool.spec.configs[]` define different disks.
...
| `VSphereMachineConfigPool.spec.configs[].persistentDisks[]` | Persistent data disks | Extra VMDKs bound to the node slot and reattached to the replacement VM, such as `/var/cpaas`. Separate from the system disk. |
| `VSphereMachineConfigPool.spec.configs[].ephemeralDisks[]` | Ephemeral data disks | Extra VMDKs created empty with the VM and deleted with it, such as `/var/lib/kubelet`, `/var/lib/containerd`, and `/var/lib/etcd`. Separate from the system disk. |

Do not add the data disk sizes to `diskGiB`. For example, if the OS image has a 100 GiB system disk and the node also needs a 100 GiB persistent disk plus three ephemeral disks, use a system disk value that is at least 100 GiB and declare the data disks in `persistentDisks` and `ephemeralDisks`. Setting `diskGiB` to the combined size would create an oversized system disk in addition to the data disks.

### Standard data disks \{#standard-data-disks}

Each node role requires a set of dedicated data disks. The disks listed below are the minimum required disks. You can append additional disks to either list if your workload requires them. An allocated slot still accepts new entries in both lists — they take effect when the VM is next recreated — but it never accepts removing or reshaping a disk, so plan sizes and mount paths before the first Machine takes the slot.

The split follows the same model as the Huawei Cloud Stack and Huawei DCS providers: only platform state that must survive VM replacement (`/var/cpaas`) is a persistent disk, while the rebuildable runtime directories (`/var/lib/kubelet`, `/var/lib/containerd`, `/var/lib/etcd`) are recreated with every VM.

:::warning
**The split applies to new clusters only**

`ephemeralDisks[]` is new in provider `v1.0.17`, so this layout is a creation-time choice. A cluster created on an earlier provider declares `/var/cpaas`, `/var/lib/containerd`, and `/var/lib/etcd` as persistent disks, with `wipeFilesystem: true` on the etcd disk, and it keeps that layout. A provider upgrade migrates no disk, and a slot that is already `InUse` or `Released` rejects the removal or the reshaping of a disk in either list, so a declared disk cannot be moved from one list to the other. See [Upgrading the Provider](../install/vmware-vsphere.mdx#upgrade-the-provider).
:::
...
| `mountOptions` | No | (empty) | Mount options applied when `mountPath` is set. |
| `fsFormat` | No | `ext4` (when `mountPath` is set) | Filesystem format. Ignored when `mountPath` is empty. |
| `wipeFilesystem` | No | `false` | When `true`, disk content is wiped on the first boot of a new VM. Reboots and manual service restarts are not affected. The standard disk set does not use it: the only persistent disk, `/var/cpaas`, is meant to keep its contents across replacement, and every disk that must start empty is declared in `ephemeralDisks[]`. |
| `unitNumber` | No | (controller-assigned) | SCSI unit that keeps guest device ordering stable across VM recreations. Between `0` and `15`, never `7` (reserved for the SCSI controller), unique within the slot, and immutable once the slot is allocated. |
| `datastore` | No | (pool default) | Override the datastore for this disk. |
| `storagePolicy` | No | (pool default) | Override the vSphere storage policy for this disk. |

#### Ephemeral disk fields

`ephemeralDisks[]` mirrors the guest-facing fields of a persistent disk — `name`, `sizeGiB`, `mountPath`, `mountOptions`, `fsFormat`, `datastore`, and `storagePolicy` — and drops the persistence fields. There is no `wipeFilesystem`, because the backing VMDK is always newly created and formatted from scratch, and no `unitNumber`, because the controller assigns the SCSI unit at clone time and reports it in `status.ephemeralDiskStatuses[]`.

Disk names and mount paths share one namespace across both lists within a slot. For the field-by-field comparison and the guidance on which list to use, see [Choose persistent or ephemeral](../create-cluster/vmware-vsphere.mdx#choose-persistent-or-ephemeral).

#### Persistent disk lifecycle \{#persistent-disk-lifecycle}

Persistent disks are owned by a slot in one specific `VSphereMachineConfigPool`, not by the short-lived VM. A slot in another pool is a different allocation even when it declares the same hostname, IP address, or disk names. The expected replacement lifecycle is:
...
4. If the slot remains unused until `releaseDelayHours` expires, the controller can make it generally available and reclaim persistent VMDKs only after it confirms they are no longer attached. Asynchronous progress appears in `status.persistentDiskStatuses[].phase`.
...
- Size the datastore for both lists. Ephemeral VMDKs occupy datastore space for as long as their VM exists, but they are deleted with it, so they never accumulate across pools and never hold a slot release. Only persistent VMDKs survive a teardown and wait for provider-managed reclaim.
...
| `reclaimStatus.*` | Deprecated. The provider folds it into `status.persistentDiskStatuses[]` on upgrade and then clears it. |

Read per-disk state from `status.persistentDiskStatuses[]`. Its `phase` field carries `Creating`, `Attached`, `Available`, `Reclaiming`, `Reclaimed`, or `Error`, and each entry also reports `lastError`, `retryAfter`, `taskRef`, and the owning machine. The `status.configStatuses[].reclaimStatus` subtree and the `spec.configs[].persistentDisks[].volumePath` and `.diskUUID` fields are deprecated, frozen, and scheduled for removal. Existing pools are migrated onto `persistentDiskStatuses[]` by the provider itself; see [Upgrading the Provider](../install/vmware-vsphere.mdx#upgrade-the-provider).
...
:::warning
Do not manually delete a VMDK while its slot is `Released`, or while its `persistentDiskStatuses[].phase` is `Reclaiming` or `Error`. A replacement Machine may still reuse it, or the provider may be retrying a verified reclaim operation. Confirm the pool state, retention window, attachment state, and controller status before performing any manual datastore cleanup.
:::

#### Control plane nodes (4 disks per node)

| Disk Name | Mount Path | Disk List | Purpose | Recommended Minimum Size | Placeholder |
|-----------|------------|-----------|---------|--------------------------|-------------|
| `var-cpaas` | `/var/cpaas` | `persistentDisks[]` | Stores logs, audit data, and other platform data | 100 GiB | `<cp_var_cpaas_size_gib>` |
| `var-lib-kubelet` | `/var/lib/kubelet` | `ephemeralDisks[]` | Stores kubelet working data, including pod volumes and plugin sockets | 100 GiB | `<cp_var_lib_kubelet_size_gib>` |
| `var-lib-containerd` | `/var/lib/containerd` | `ephemeralDisks[]` | Stores containerd runtime data (images, layers, snapshots) | 100 GiB | `<cp_var_lib_containerd_size_gib>` |
| `var-lib-etcd` | `/var/lib/etcd` | `ephemeralDisks[]` | Stores etcd data. The disk is created empty for every new VM, so a replacement control-plane node never carries stale member data into `kubeadm join`. | 20 GiB | `<cp_var_lib_etcd_size_gib>` |

#### Worker nodes (3 disks per node)

| Disk Name | Mount Path | Disk List | Purpose | Recommended Minimum Size | Placeholder |
|-----------|------------|-----------|---------|--------------------------|-------------|
| `var-cpaas` | `/var/cpaas` | `persistentDisks[]` | Stores logs, audit data, and other platform data | 100 GiB | `<worker_var_cpaas_size_gib>` |
| `var-lib-kubelet` | `/var/lib/kubelet` | `ephemeralDisks[]` | Stores kubelet working data, including pod volumes and plugin sockets | 100 GiB | `<worker_var_lib_kubelet_size_gib>` |
| `var-lib-containerd` | `/var/lib/containerd` | `ephemeralDisks[]` | Stores containerd runtime data (images, layers, snapshots) | 100 GiB | `<worker_var_lib_containerd_size_gib>` |
...
| CP `/var/lib/kubelet` size GiB | `<cp_var_lib_kubelet_size_gib>` | Yes | Recommended minimum 100 GiB. | `100` | - |
| CP `/var/lib/containerd` size GiB | `<cp_var_lib_containerd_size_gib>` | Yes | Recommended minimum 100 GiB. | `100` | - |
| CP `/var/lib/etcd` size GiB | `<cp_var_lib_etcd_size_gib>` | Yes | Recommended minimum 20 GiB. | `20` | - |
| Worker `/var/cpaas` size GiB | `<worker_var_cpaas_size_gib>` | Yes | Recommended minimum 100 GiB. | `100` | - |
| Worker `/var/lib/kubelet` size GiB | `<worker_var_lib_kubelet_size_gib>` | Yes | Recommended minimum 100 GiB. | `100` | - |
...
5. The external LoadBalancer satisfies the Layer 4 listener, backend, health-check, ownership, and reachability contract. If the cluster uses the Self-built VIP instead, the [Self-built VIP prerequisites](../create-cluster/vmware-vsphere.mdx#self-built-vip-prerequisites) are met and `<vrid>` and `<vip_interface>` are decided.
...
14. The baseline system disk and data disk sizing is confirmed, each required disk is declared in the correct list (`/var/cpaas` persistent; `/var/lib/kubelet`, `/var/lib/containerd`, and `/var/lib/etcd` ephemeral), and data disk sizes are not added into `diskGiB`.
