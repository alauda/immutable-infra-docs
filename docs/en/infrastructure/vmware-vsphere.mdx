...
| CoreDNS image tag | `<dns_image_tag>` | Yes | Use the tag approved for the Kubernetes version. Keep the `-v<acp>` suffix: the provider derives `dns.imageRepository` from it, so a tag without the suffix leaves the repository undefined. | `1.14.3-v4.4.6` | - |
...
# Kube-OVN chart. Use the repository that matches the target chart version:
#   v4.4 and later    -> acp/chart-kube-ovn
#   earlier than v4.4 -> acp/chart-cpaas-kube-ovn
curl -sk "https://${CLUSTER_REGISTRY_ADDRESS}/v2/acp/chart-kube-ovn/tags/list"

# CoreDNS. Use the repository that matches the target ACP version:
#   ACP v4.4 and later -> acp/coredns
#   earlier            -> tkestack/coredns
curl -sk "https://${IMAGE_REGISTRY_ADDRESS}/v2/acp/coredns/tags/list"
