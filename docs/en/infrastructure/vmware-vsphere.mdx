...
This section covers the external LoadBalancer endpoint, which the baseline cluster uses. Provision it before cluster creation and follow [Plan the Control Plane Endpoint](../how-to/control-plane-endpoint.mdx). A cluster can instead be created with the provider-managed Self-built VIP, which needs no external LoadBalancer; see [Control Plane Endpoint Modes](../create-cluster/vmware-vsphere.mdx#control-plane-endpoint-modes).
...
| `VSphereMachineConfigPool.spec.configs[].ephemeralDisks[]` | Ephemeral data disks | Extra VMDKs created empty with the VM and deleted with it, such as `/var/lib/containerd` and `/var/lib/etcd`. Separate from the system disk. |
...
The split follows the same model as the Huawei Cloud Stack and Huawei DCS providers: only platform state that must survive VM replacement (`/var/cpaas`) is a persistent disk, while the rebuildable runtime directories (`/var/lib/containerd`, `/var/lib/etcd`) are recreated with every VM.
...
14. The baseline system disk and data disk sizing is confirmed, each required disk is declared in the correct list (`/var/cpaas` persistent; `/var/lib/containerd` and `/var/lib/etcd` ephemeral), and data disk sizes are not added into `diskGiB`.
