...
A slot can also declare `ephemeralDisks[]`, which is recreated empty with every VM. For the field-by-field comparison and the guidance on which list to use, see [Choose persistent or ephemeral](../create-cluster/vmware-vsphere.mdx#choose-persistent-or-ephemeral).
