...
The web UI currently only creates `Interface interconnection user` credentials. To create a `Domain user` credential, use the CLI path below and set `userType` to `domain`.
:::

### Using the CLI \{#using-yaml}

Create a Secret in `cpaas-system` that stores the DCS authentication information.
...
```bash
chmod 600 dcs-credential.env

DCS_SECRET_NAME="<auth-secret-name>"

kubectl create secret generic "${DCS_SECRET_NAME}" \
  --namespace cpaas-system \
  --from-env-file=dcs-credential.env

rm -f dcs-credential.env
```

Write the values in plain text — `kubectl` base64-encodes them when it creates the Secret, so there is no separate encoding step. Each line is `key=value`. Do not quote the values: `kubectl` takes quote characters literally, so `authKey="s3cret"` stores `"s3cret"` with the quotes included. A value may contain `=`; only the first `=` on a line separates the key from the value.

Omit the `userType` line for an interface interconnection user, which is the default. Set it to `domain` for a domain user — see [Credential User Types](#user-types).

:::warning
Do not create a credential Secret with `kubectl apply`. Client-side apply stores the manifest it applied — including the DCS password — in the `kubectl.kubernetes.io/last-applied-configuration` annotation on the Secret itself. `kubectl create` writes no such annotation. This applies whether the values were written as `stringData` or as base64 `data`; base64 is an encoding, not protection.
:::

To change a credential later, patch it rather than reapplying a manifest. Author the patch file with an editor, for the same reason:

```yaml title="dcs-credential-patch.yaml"
stringData:
  authKey: <new-auth-key>
```

```bash
chmod 600 dcs-credential-patch.yaml

kubectl patch secret "${DCS_SECRET_NAME}" --namespace cpaas-system \
  --type=merge --patch-file=dcs-credential-patch.yaml

rm -f dcs-credential-patch.yaml
```
...
| Key | Description |
|-----------|-------------|
| `authUser` | DCS platform API user login name. |
| `authKey` | DCS platform API user login password. |
| `endpoint` | DCS platform API address, including the `http` or `https` scheme. **Note**: The default API port for the DCS platform is 7443 (not the common 8443). If your environment uses a custom port, confirm with your administrator. |
| `site` | DCS site ID. The provider reads it from this Secret and writes it to `DCSCluster.spec.site` when that field is left empty, so setting it here is enough. Set `DCSCluster.spec.site` explicitly only to override the Secret for one cluster. |
| `userType` | Optional. Allowed values are `interconnect` (default; used when the key is absent or empty) or `domain`. Matching is case-insensitive (`Domain`, `DOMAIN`, and `domain` all map to the same value). Any other value, including the legacy DCS magic numbers `1` and `2`, is rejected with a configuration error so a `domain` user is never silently authenticated as `interconnect`. |
