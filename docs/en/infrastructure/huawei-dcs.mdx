...
Create a Secret in `cpaas-system` that stores the DCS authentication information. Write the values into a file with restricted permissions, create the Secret from that file, then remove the file:

```bash
umask 077
cat > dcs-credential.env <<'EOF'
authUser=<auth-user>
authKey=<auth-key>
...
To change a credential later, patch it rather than reapplying a manifest:

```bash
umask 077
cat > dcs-credential-patch.yaml <<'EOF'
stringData:
  authKey: <new-auth-key>
EOF

kubectl patch secret <auth-secret-name> --namespace cpaas-system \
