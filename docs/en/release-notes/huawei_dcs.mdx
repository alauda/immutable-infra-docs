...
## v1.0.22 (2026-08-26) \{#v1-0-22\}

### New Features

- **Provider-managed Self-built VIP for DCS** — DCS Provider `v1.0.22` supports `DCSCluster.spec.controlPlaneLoadBalancer.type: internal` when paired with ACP `v4.4` or later. The provider injects a temporary bootstrap VIP for the first control-plane initialization, then creates or updates the `alive` `ModuleInfo` and waits for the VIP endpoint to become ready. Reserve an IPv4 VIP in the control-plane Layer 2 network, keep its VRID unique in that network, and use three control-plane replicas for high availability. Existing external LoadBalancer clusters are not migrated automatically.

### Enhancements

- **Version-aware system component repositories** — The provider selects the Kube-OVN chart repository from the target chart version, the CoreDNS repository from the ACP suffix in the CoreDNS tag, and the kube-proxy repository from the Kubernetes version. Kube-OVN `v4.4+` uses `acp/chart-kube-ovn`; earlier chart versions retain `acp/chart-cpaas-kube-ovn`. Kubernetes `v1.35+` uses `acp/k8s/kube-proxy`.

- **Upgrade-time system component reconciliation** — The provider watches KubeadmControlPlane version and rollout changes, configures the CoreDNS repository before control-plane creation, and reconciles CoreDNS and kube-proxy after the relevant rollout. Do not manually replace the source of provider-managed AppRelease resources.
...
- **CoreDNS image pull secret reconciliation** — CoreDNS image pull secrets are now evaluated from the CoreDNS Deployment instead of unrelated secrets on the sentry ServiceAccount.

- **Dependency security maintenance** — Updated non-standard-library Go dependencies, including CEL and Kubernetes libraries, to address vulnerability findings and align with the supported platform baseline.
...
- **Alauda OS compatibility for ACP v4.3.2 and later** — The provider reliably applies the configured primary node address on multi-NIC nodes during bootstrap. DCS Provider `v1.0.21` is required when a DCS cluster uses the Alauda OS image for ACP `v4.3.2` or later, including ACP `v4.4.0` and later. Select the image published for the same ACP release. Do not pair DCS Provider `v1.0.21` or later with an Alauda OS image for an ACP release earlier than `v4.3.2`. See [Alauda OS and Provider Compatibility](../overview/providers/huawei-dcs.mdx#alauda-os-and-provider-compatibility).
