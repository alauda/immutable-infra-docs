...
- This can lead to unpredictable failures during worker node provisioning or scaling
:::

:::info
**Shared Storage for Ignition**

If the datastore used for system volume and ignition does not support file upload operations, you must provide a shared storage solution that supports multi-host mounting (e.g., NFS). This ensures that ignition configuration files can be accessed by worker node VMs regardless of which physical machine they are created on.
:::
