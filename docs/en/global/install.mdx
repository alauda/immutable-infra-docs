...
| `console.host` | Local `global` HA VIP list | Local `global` control-plane VIP list | Empty list, `[]` | Local `global` control-plane VIP list |
...
| `cluster.features.ha` | Required, points to the local HA VIP with `isThirdParty: true` | Required, points to the local control-plane VIP with `isThirdParty: true` | Not set; HCS ELB is declared in `HCSCluster` | Required, points to the bare-metal control-plane VIP with `isThirdParty: true` |
...
Set `console.host` and `cluster.features.ha.vip` to the same control plane VIP that the `global` manifest declares in `VSphereCluster.spec.controlPlaneEndpoint.host`, whether that address is served by an external LoadBalancer or by the Self-built VIP. Do not use the platform domain in `console.host`; use `console.globalHost` for the platform access address.
...
| VMware vSphere | Set `console.host`, `cluster.features.ha.vip`, and `VSphereCluster.spec.controlPlaneEndpoint.host` to the primary HA VIP used by the primary manifest. Create the VMware vSphere `dcs-import-extra-resources` ConfigMap from Step 7 and keep `global-vsphere-credentials` aligned with `VSphereCluster.spec.identityRef.name`. | Set `console.host`, `cluster.features.ha.vip`, and `VSphereCluster.spec.controlPlaneEndpoint.host` to the standby HA VIP used by the standby manifest. Create the VMware vSphere `dcs-import-extra-resources` ConfigMap from Step 7 and keep `global-vsphere-credentials` aligned with `VSphereCluster.spec.identityRef.name`. |
