...
| `cluster.features.ha` | Required, points to the local HA VIP with `isThirdParty: true` | Required, points to the control plane VIP with `isThirdParty: true`. The same for both endpoint modes; the VIP is also declared in `VSphereCluster.spec.controlPlaneEndpoint.host` | Not set; HCS ELB is declared in `HCSCluster` | Required, points to the bare-metal control-plane VIP with `isThirdParty: true` |
