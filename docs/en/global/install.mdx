...
Apply the same `baremetal-dr-rules` ConfigMap to the primary and standby. Use the template in [Bare Metal etcd-sync Rules](./disaster_recovery.mdx#synchronization-rules). Replace the plan, inventory, and registration placeholders with current values from both clusters. Do this before the first synchronization and whenever a Global node or plan is added.
