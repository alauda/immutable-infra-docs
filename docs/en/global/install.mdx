...
Huawei DCS, VMware vSphere, Huawei Cloud Stack, and Bare Metal require this ConfigMap for both normal and disaster recovery `global` installations. For Huawei DCS, the provider resources are migrated by the built-in flow, but the Secret referenced by `DCSCluster.spec.credentialSecretRef.name` must be imported explicitly.

<Tabs>
  <Tab label="Huawei DCS">

Create and apply the DCS import ConfigMap before you trigger the installer. Include the Secret referenced by `DCSCluster.spec.credentialSecretRef.name`. The DCS provider resources are migrated by the built-in flow, but the credential Secret must exist in the final `global` cluster so the DCS provider can continue to call the DCS API after handoff.
...
After the installer reports success, the `global` cluster runs its own Cluster API providers. Once the general verification and the provider-specific handoff gate below pass, remove the temporary bootstrap cluster from the KIND host — delete only the local bootstrap cluster (`minialauda`) and its KIND container network.
