...
#### Bare Metal verification before decommissioning either bootstrap cluster \{#bare-metal-dr-verification}
...
For Bare Metal, do not reduce acceptance to the `ready` signal alone. A runtime token or CA mismatch, a stale bootstrap endpoint in the persistent OEM file, a failed plan probe, or an incorrect permission boundary means fresh handoff is incomplete even when `data.ready` is `true`.
...
For Bare Metal, apply this ConfigMap **before** you install `etcd-sync`, on both the primary and the standby.
...
### Verify the Bare Metal dynamic rule source

For Bare Metal, before opening the sync path, confirm that the rule source reports `accepted` and appears in `ConfigMap/etcd-sync-rule-snapshot`:
...
## Prepare the global Cluster as a Management Cluster (Bare Metal) \{#prepare-global-as-management-cluster}
