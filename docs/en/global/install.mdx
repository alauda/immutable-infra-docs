...
- Keep the existing `VSphereMachineConfigPool` references during Machine replacement. A replacement can immediately reuse a `Released` slot and its VMDKs only within the same existing pool; a different pool name creates an independent disk set. Before retrying a torn-down installation on a capacity-constrained datastore, verify that earlier pools and their VMDKs have completed provider-managed deletion. See [Persistent disk lifecycle](../create-cluster/vmware-vsphere/parameter-checklist.mdx#persistent-disk-lifecycle).
