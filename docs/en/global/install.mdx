...
On arm64, provision more CPU and memory than the x86_64 figures above, following the resource-planning convention in <ExternalSiteLink name="acp" href="/install/prepare/prerequisites.html" children="Prerequisites" />.

### Operating System \{#bootstrap-host-os}

The bootstrap host runs a traditional operating system, not Alauda OS. Use a distribution and kernel from the validated set in <ExternalSiteLink name="acp" href="/install/prepare/node_preprocessing.html#supported_os_and_kernels" children="Supported OS and Kernel Versions" />. The host needs Bash and enough privilege to run `setup.sh` as `root`.
