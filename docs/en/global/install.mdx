...
- Synchronize the clock on every `global` host before the live-ISO boot, and keep the spread across the control-plane hosts within the platform's 10-second requirement. This control plane generates its PKI and forms etcd during the same provisioning pass, so a wrong clock surfaces at control-plane bring-up rather than at registration. See [Host Time Synchronization](../create-cluster/bare-metal.mdx#host-time-synchronization).
