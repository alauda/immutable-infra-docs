...
Import the Bare Metal Alauda OS images into the same local registry. The provider package does not contain them. The Bare Metal OS download is an archive of one OCI image layout that holds two images with the same tag, and both are required — see [Image Format by Provider](../overview/os-support-matrix.mdx#image-format-by-provider):

| Image | Used by |
|---|---|
| `baremetal-base-image` | `elemental-image-catalog`, when a host is reprovisioned into a `global` node. |
| `baremetal-base-image-iso` | `SeedImage.spec.baseImage`, when the installation ISO for the `global` hosts is built. |

Import the archive into a dedicated containerd namespace on the bootstrap host with `ctr`, and list the image names it creates. `/var/lib/containerd` needs free space of at least the archive size:
...
The output lists both images with the same tag:
...
Set `OS_IMAGE_TAG` to that tag, then push both images and confirm that the registry lists the tag for each of them:
...
If the bootstrap Registry requires authentication, pass its credentials to both commands with `-u <username>:<password>`. Keep the images in the `bare-metal-os` namespace: [Upload the base image to the global Registry](#upload-os-images-to-global-registry) pushes them again after installation.

Create and apply the AppRelease resources for the Kubeadm provider and the Bare Metal provider.

The Bare Metal provider AppRelease also fills the image catalog. The chart creates the `elemental-image-catalog` ConfigMap with no entries, so `provider.imageCatalog.images` must map `${K8S_VERSION}` to the `baremetal-base-image` imported above; the chart prefixes the repository with `global.registry.address`. Without that entry, every `global` `BaremetalMachine` enters `Failed` with `ImageResolved=False / Reason=ImageCatalogMiss` and no node is provisioned.
...
The catalog output must contain `${K8S_VERSION}` as a key. Empty output or `{}` means the `provider.imageCatalog.images` value did not reach the release; fix the AppRelease before you apply the `global` manifest.
...
Create the DCS API credential `Secret` before you assemble the manifest, by following [Cloud Credentials](../infrastructure/huawei-dcs.mdx#using-yaml). Do not author it as YAML inside the manifest: the manifest is applied with `kubectl apply` in Step 5, which records the credential in the Secret's `kubectl.kubernetes.io/last-applied-configuration` annotation. The manifest references it through `DCSCluster.spec.credentialSecretRef`.
...
- With DCS Provider `v1.0.22`, use an external LoadBalancer and set `DCSCluster.spec.controlPlaneLoadBalancer.type: external`, or use `type: internal` Self-built VIP when paired with ACP `v4.4+`. For internal mode, reserve an IPv4 VIP in the control-plane Layer 2 network and use the YAML workflow in [Creating Clusters on Huawei DCS](../create-cluster/huawei-dcs.mdx#self-built-vip). For external mode, validate the listener, backends, health check, and reachability in [Plan the Control Plane Endpoint](../how-to/control-plane-endpoint.mdx).
...
- For a normal non-DR deployment, do not set `DCSCluster.spec.encryptionProviderConfigRef`. Keep `clusterConfiguration.apiServer.extraArgs.encryption-provider-config` as the appendix has it: the DCS provider writes `/etc/kubernetes/encryption-provider.conf` onto every control-plane node itself — a random key for the first machine, the running API server's file for the rest — so the argument resolves and the `global` cluster runs with etcd secret encryption-at-rest. Do not add that file to `KubeadmControlPlane.spec.kubeadmConfigSpec.files`; the provider drops any entry for that path, so one added there is silently unused. See [How the Encryption Provider Configuration Is Delivered](../create-cluster/huawei-dcs.mdx#encryption-provider-delivery).
...
Create the two credential Secrets before you assemble the manifest, and keep them out of it. The manifest is applied with `kubectl apply` in Step 5, which would record their contents in each Secret's `kubectl.kubernetes.io/last-applied-configuration` annotation.

* The vCenter credential, which **must** be named `global-vsphere-credentials` — the import ConfigMap in Step 7 and the DR section hardcode this exact name. Create it as described in [Creating Clusters on VMware vSphere](../create-cluster/vmware-vsphere.mdx), using that fixed name, and reference it from `VSphereCluster.spec.identityRef.name`.
* The vSphere CPI credential, referenced by the `ClusterResourceSet`. Create it as described in the same guide.

The VMware vSphere `global` manifest must then contain the following resources in the `cpaas-system` namespace:
...
| `ClusterResourceSet` and `ConfigMap` for the vSphere CPI | Delivers the vSphere CPI resources to the `global` cluster after the API server becomes reachable. The `ClusterResourceSet` also references the CPI credential Secret created above. |
...
- Select the control plane endpoint mode before you apply the manifest; it cannot be changed afterwards. For an external LoadBalancer, provision and validate it first by using the contract in [Plan the Control Plane Endpoint](../how-to/control-plane-endpoint.mdx). With VMware vSphere Provider `v1.0.17` or later on ACP `v4.4` or later, the `global` cluster can instead use an `internal` Self-built VIP: complete [Self-built VIP prerequisites](../create-cluster/vmware-vsphere.mdx#self-built-vip-prerequisites), then add the `spec.controlPlaneLoadBalancer` block from [Enable the Self-built VIP](../create-cluster/vmware-vsphere.mdx#enable-self-built-vip) to the `global` `VSphereCluster`. With `v1.0.16` or earlier, only the external LoadBalancer is available.

  The Self-built VIP prerequisite commands read `ModulePlugin/alive` from a running `global` cluster, which does not exist yet during the bootstrap installation. Confirm from the installation package instead that it carries the `alive` plugin.
...
- Add `Cluster.metadata.annotations["cpaas.io/registry-address"]` with `${NODE_REGISTRY_ADDRESS}`.
- Set `SeedImage.spec.baseImage` to `${BOOTSTRAP_REGISTRY_ADDRESS}/tkestack/baremetal-base-image-iso:${OS_IMAGE_TAG}`, the ISO image imported in Step 3. The reprovision image comes from the `elemental-image-catalog` entry set in Step 3; the provider replaces the registry portion of that entry with the `cpaas.io/registry-address` annotation, so the hosts pull it from `${NODE_REGISTRY_ADDRESS}`.
...
- Set the clock from firmware on every `global` host before the live-ISO boot, and keep the spread across the hosts within the platform's 10-second requirement. See [Host Time Synchronization](../create-cluster/bare-metal.mdx#host-time-synchronization).
...
Every provider uses this ConfigMap to import the IaaS credential `Secret` into the new `global` cluster. For VMware vSphere, Huawei Cloud Stack, and Bare Metal it also imports the provider's Cluster API resources; for Huawei DCS those resources are migrated by the built-in flow, so the DCS ConfigMap only needs the credential `Secret` entry for a normal installation. VMware vSphere, Huawei Cloud Stack, and Bare Metal require it for both normal and disaster recovery `global` installations.

For a disaster recovery installation, the two providers that deliver the encryption provider configuration through a Secret reference — Huawei DCS (`DCSCluster.spec.encryptionProviderConfigRef`) and Bare Metal (`BaremetalCluster.spec.encryptionProviderConfigRef`) — must also import that Secret through this ConfigMap. The built-in DCS migration flow moves Cluster API resources only; it does not copy the referenced Secret.

<Tabs>
  <Tab label="Huawei DCS">

The DCS provider's Cluster API resources are migrated by the built-in flow, so for a normal installation the ConfigMap only needs to import the credential `Secret` referenced by `DCSCluster.spec.credentialSecretRef.name`. Use the same Secret name you set in the `global` manifest.
...
For a DR installation, use the form below instead. It adds the encryption-provider `Secret` referenced by `DCSCluster.spec.encryptionProviderConfigRef.name`, which must reach the final `global` cluster. `DCS_ENCRYPTION_PROVIDER_SECRET` comes from [Prepare Shared DR Variables](#prepare-shared-dr-variables); the first line fails loudly if it is not exported, because the heredoc would otherwise expand it to an empty Secret name.
...
Check the rendered ConfigMap before continuing: both `names` entries must be non-empty, and the second must match `DCSCluster.spec.encryptionProviderConfigRef.name` exactly.
...
| `cluster.features.ha` | Required, `vip` is `${CONTROL_PLANE_VIP}` with `isThirdParty: true` | Required, `vip` is `${CONTROL_PLANE_VIP}` with `isThirdParty: true`. The same for both endpoint modes; the VIP is also declared in `VSphereCluster.spec.controlPlaneEndpoint.host` | Not set; HCS ELB is declared in `HCSCluster` | Required, `vip` is `${CONTROL_PLANE_VIP}` with `isThirdParty: true` |
| `hostIP` | Current bootstrap host IP | Current bootstrap host IP | Current bootstrap host IP | Current bootstrap host IP |

Each `console.host` entry is a bare address: no `https://` prefix and no port. The ports come from `console.httpPort` and `console.httpsPort`.

<Tabs>
  <Tab label="Huawei DCS">

The DCS installer request includes the `global` control-plane VIP. With an external LoadBalancer, this is the externally provisioned VIP. With DCS Provider `v1.0.22+` and ACP `v4.4+` `Self-built VIP`, use the reserved VIP that the provider-managed `alive` runtime owns.
...
Set `console.host` to the local `global` control-plane VIP, `${CONTROL_PLANE_VIP}`. Do not use the platform domain in `console.host`; use `console.globalHost` for the platform access address. `cluster.features.ha.vip` uses the same address.

  </Tab>
  <Tab label="VMware vSphere">

VMware vSphere uses the same installer endpoint path as DCS. The request body carries the control plane VIP in `console.host` and `cluster.features.ha`, the same way for an external LoadBalancer and for a Self-built VIP; the same address is also declared in `VSphereCluster.spec.controlPlaneEndpoint.host`. The cluster CIDRs are not sent here — they are declared in the VMware vSphere `Cluster` manifest.
...
Set `console.host` and `cluster.features.ha.vip` to the local `global` control-plane VIP, `${CONTROL_PLANE_VIP}`, whether that address is served by an external LoadBalancer or by the Self-built VIP. Do not use the platform domain in `console.host`; use `console.globalHost` for the platform access address. `VSphereCluster.spec.controlPlaneEndpoint.host` uses the same address.

  </Tab>
  <Tab label="Huawei Cloud Stack">

HCS uses the same installer endpoint path as DCS, but its request body does not include `cluster.features.ha`. The control-plane VIP is owned by the HCS ELB declared in `HCSCluster.spec.controlPlaneLoadBalancer`.
...
Set `console.host` to the local `global` control-plane VIP, `${CONTROL_PLANE_VIP}`. Do not use the platform domain in `console.host`; use `console.globalHost` for the platform access address. `HCSCluster.spec.controlPlaneLoadBalancer.vipAddress` uses the same address.
...
### Prepare Shared DR Variables \{#prepare-shared-dr-variables}
...
### Add Provider-Specific DR Fields \{#add-provider-specific-dr-fields}

<Tabs>
  <Tab label="Huawei DCS">

Create the encryption provider Secret in the bootstrap cluster on both installation environments. The Secret must be in the same namespace as `DCSCluster` and must contain a key named `encryption-provider.conf`.
...
DCS uses `DCSCluster.spec.encryptionProviderConfigRef` to deliver the disaster recovery encryption provider configuration. The DCS provider reads this Secret every time it builds control-plane bootstrap data. Do not add `/etc/kubernetes/encryption-provider.conf` to `KubeadmControlPlane.spec.kubeadmConfigSpec.files` for the DCS DR path.

This Secret must also be included in the DCS `dcs-import-extra-resources` ConfigMap from Step 7. It cannot remain only in the bootstrap cluster because the handed-off `global` cluster keeps the `DCSCluster` object and its `encryptionProviderConfigRef`, and must also have the referenced Secret for later provider reconciliation. The built-in DCS migration flow moves the Cluster API resources but does not copy this Secret.

Create the DCS `dcs-import-extra-resources` ConfigMap on both installation environments, using the DR form of the ConfigMap in Step 7 rather than the normal one. Set `PROVIDER_SECRET_NAME` to the same Secret name used by `DCSCluster.spec.credentialSecretRef.name`, and keep `DCS_ENCRYPTION_PROVIDER_SECRET` aligned with `DCSCluster.spec.encryptionProviderConfigRef.name`.

<Directive type="warning" title="A missing Secret only fails later, when a control-plane machine is replaced">
  When `DCSCluster.spec.encryptionProviderConfigRef` is set, the DCS provider requires the referenced Secret and does not fall back to generating or reading a configuration. Reconciles that do not create a control-plane machine are unaffected, so an installation whose Secret stayed in the bootstrap cluster looks healthy until something needs a new control-plane machine — a Kubernetes upgrade, a `KubeadmControlPlane` rolling update, or a control-plane node replacement. The new `Machine` then stays in `Provisioning` with no VM created, and the provider logs `failed to create vm: failed to resolve user data: get encryption provider config secret`.

  Check both sides of an existing DR pair, and backfill the Secret if it is missing:
  ...
  Recreate it from the same `encryption-provider.conf` the installation used, so new control-plane machines keep the key the running API servers already use. If that file is no longer on the bootstrap host, copy it from a running control-plane node of the same cluster (`/etc/kubernetes/encryption-provider.conf`). A different key breaks decryption of the etcd data the pair already holds.
</Directive>
...
|----------|----------------------|----------------------|
| Huawei DCS | Set `console.host` to the primary control-plane VIP. `cluster.features.ha.vip` uses the same address. Create the DCS `dcs-import-extra-resources` ConfigMap from Step 7 and keep `PROVIDER_SECRET_NAME` aligned with `DCSCluster.spec.credentialSecretRef.name` and `DCS_ENCRYPTION_PROVIDER_SECRET` aligned with `DCSCluster.spec.encryptionProviderConfigRef.name`. | Set `console.host` to the standby control-plane VIP. `cluster.features.ha.vip` uses the same address. Create the DCS `dcs-import-extra-resources` ConfigMap from Step 7 and keep `PROVIDER_SECRET_NAME` aligned with `DCSCluster.spec.credentialSecretRef.name` and `DCS_ENCRYPTION_PROVIDER_SECRET` aligned with `DCSCluster.spec.encryptionProviderConfigRef.name`. |
| VMware vSphere | Set `console.host` to the primary control-plane VIP. `cluster.features.ha.vip` and `VSphereCluster.spec.controlPlaneEndpoint.host` use the same address. Create the VMware vSphere `dcs-import-extra-resources` ConfigMap from Step 7 and keep `global-vsphere-credentials` aligned with `VSphereCluster.spec.identityRef.name`. | Set `console.host` to the standby control-plane VIP. `cluster.features.ha.vip` and `VSphereCluster.spec.controlPlaneEndpoint.host` use the same address. Create the VMware vSphere `dcs-import-extra-resources` ConfigMap from Step 7 and keep `global-vsphere-credentials` aligned with `VSphereCluster.spec.identityRef.name`. |
| Huawei Cloud Stack | Set `console.host` to the primary control-plane VIP. `HCSCluster.spec.controlPlaneLoadBalancer.vipAddress` uses the same address. Create the HCS `dcs-import-extra-resources` ConfigMap from Step 7 and keep `HCS_SECRET_NAME` aligned with `HCSCluster.spec.identityRef.name`. | Set `console.host` to the standby control-plane VIP. `HCSCluster.spec.controlPlaneLoadBalancer.vipAddress` uses the same address. Create the HCS `dcs-import-extra-resources` ConfigMap from Step 7 and keep `HCS_SECRET_NAME` aligned with `HCSCluster.spec.identityRef.name`. |
| Bare Metal | Set `console.host` to the primary control-plane VIP. `cluster.features.ha.vip`, `BaremetalCluster.spec.controlPlaneLoadBalancer.host`, and `handoffHook.controlPlaneVIP` use the same address. Set `handoffHook.directAPIServer: true`, `elemental.systemAgent.splitAuthEnabled: true`, and `elemental.systemAgent.sharedAuthReadOnly: false`. Create the Bare Metal `dcs-import-extra-resources` ConfigMap from Step 7. | Set `console.host` to the standby control-plane VIP. `cluster.features.ha.vip`, `BaremetalCluster.spec.controlPlaneLoadBalancer.host`, and `handoffHook.controlPlaneVIP` use the same address. Set `handoffHook.directAPIServer: true`, `elemental.systemAgent.splitAuthEnabled: true`, and `elemental.systemAgent.sharedAuthReadOnly: true`. Create the Bare Metal `dcs-import-extra-resources` ConfigMap from Step 7. |

For the primary cluster, make sure the platform domain resolves to the primary control-plane VIP. In Step 8, set `hostIP` to the primary bootstrap host IP, and set `console.host` and the provider fields listed in the table above to the primary control-plane VIP.

After the primary cluster installation succeeds, switch the platform domain to the standby control-plane VIP as required by the DR procedure. Then install the standby cluster. This DNS switch before the standby installation is required because several platform resources are rendered with the platform domain and must resolve to the standby entrance while the standby installer runs. In Step 8 on the standby bootstrap host, set `hostIP` to the standby bootstrap host IP. Set `console.host` and the provider fields listed in the table above to the standby control-plane VIP. For Bare Metal, keep `REGISTRY_DOMAIN` at `${PLATFORM_HOST}:11443` — the same value the primary installation used. Get `INSTALLER_IP` from the `cpaas-installer` Pod on the standby bootstrap host; do not reuse the primary bootstrap host value.
...
Read the Registry address and password of the cluster you are preparing from its own
`ProductBase` and `cpaas-system/registry-admin` Secret:
...
### Upload the base image to the global Registry \{#upload-os-images-to-global-registry}

Workload `SeedImage` builds run on the `global` cluster and pull the base ISO from the Registry
recorded in `ProductBase.spec.registry.address`. After handoff, reprovisioning a `global` node also
pulls `baremetal-base-image` from that Registry, because the installer rewrites the `global`
cluster's `cpaas.io/registry-address` annotation to it. The images imported in Step 3 live in the
bootstrap Registry only, so the ISO build fails with `MANIFEST_UNKNOWN` until you push both of them
to the `global` Registry. On the bootstrap host, push them from the `bare-metal-os` containerd
namespace. `--skip-verify` requires `--local`:
...
Each response must list `${OS_IMAGE_TAG}`. After that, remove the local copies with
`ctr -n bare-metal-os images rm "baremetal-base-image:${OS_IMAGE_TAG}" "baremetal-base-image-iso:${OS_IMAGE_TAG}"`.
If the images are no longer on the bootstrap host, import them from the OS archive instead, as
described in [OS Images Imported and Image Catalog Populated](../create-cluster/bare-metal.mdx#os-images-and-image-catalog).
...
`elemental-image-catalog`. The provider chart creates this ConfigMap with no version entries, and
the `global` cluster does not inherit the entry you set on the bootstrap release. Until you add an
entry, the first `BaremetalMachine` of a workload cluster fails immediately:
...
Add one entry per Kubernetes version that this `global` cluster manages, including `${K8S_VERSION}`
for its own nodes. Patch the existing ConfigMap rather than replacing it, so that entries added
earlier are kept:
...
<Directive type="warning" title="ImageCatalogMiss does not recover on its own">
  ...
  Delete the `Machine` so that its owner, `KubeadmControlPlane` or the `MachineSet`, recreates it.
  Populate the catalog before you apply the first workload cluster manifest to avoid this entirely.
</Directive>
...
<Directive type="info" title="Verify the DCS Secrets reached the global cluster">
  ...
  For a DR installation, run the same check for the Secret named in `DCSCluster.spec.encryptionProviderConfigRef`. The `DCSCluster` object survives the handoff with that reference, so once the bootstrap cluster is gone a missing Secret makes every later control-plane machine creation fail — see [Add Provider-Specific DR Fields](#add-provider-specific-dr-fields).
</Directive>
...
1. **DCS API access, and the credential Secret created from it** — the endpoint (`https://<host>:7443`), an `administrator` user and password, and the site ID. All four go into the credential Secret; the provider reads the site ID from there and writes it to `DCSCluster.spec.site` when that field is left empty, so the manifest below does not set it.
   ...
   Create the Secret named `${PROVIDER_SECRET_NAME}` in `cpaas-system` by following [Cloud Credentials](../infrastructure/huawei-dcs.mdx#using-yaml). Do not author it as YAML and apply it: `kubectl apply` records the password in the Secret's `kubectl.kubernetes.io/last-applied-configuration` annotation.
...
4. **IPs and the API endpoint** — prepare three free node IPs (with gateway and DNS) for the control plane; these fill the `DCSIpHostnamePool`. Then choose either an external LoadBalancer that follows [the control plane endpoint contract](../how-to/control-plane-endpoint.mdx) and forwards TCP `6443` to the three nodes, or a separate IPv4 VIP in the same Layer 2 network for DCS Provider `v1.0.22+` Self-built VIP with ACP `v4.4+`; the selected endpoint host and port fill `DCSCluster.spec.controlPlaneLoadBalancer` and `controlPlaneEndpoint`. See [Creating Clusters on Huawei DCS](../create-cluster/huawei-dcs.mdx).
...
|---|---|
| `${K8S_VERSION}`, `${CONTROL_PLANE_VIP}`, `${PLATFORM_HOST}`, `${NODE_REGISTRY_ADDRESS}`, `${CLUSTER_CIDR}`, `${SERVICE_CIDR}`, `${PROVIDER_SECRET_NAME}` | Exported in Step 1. |
| `<dcs-api-user>` / `<dcs-api-password>` / `<dcs-api-host>` / `<dcs-site-id>` | DCS platform credentials and site. They do not appear in the manifest — they go into the credential file used to create `${PROVIDER_SECRET_NAME}` in the prerequisites. See [Cloud Credentials](../infrastructure/huawei-dcs.mdx#using-yaml). |
...
| `192.0.2.x`, `<kube-ovn-join-cidr>`, `<kube-ovn-chart-version>` | Node IPs/gateway/DNS for your network; a `/16` Kube-OVN join CIDR that does not overlap the pod / service CIDRs or another cluster's join CIDR; and the kube-ovn chart version from the [OS Support Matrix](../overview/os-support-matrix.mdx). |
...
<Directive type="info" title="Secret encryption and disaster recovery">
  This example runs with etcd secret encryption-at-rest enabled. `apiServer.extraArgs.encryption-provider-config` stays in the `clusterConfiguration` you copy from the appendix, and the DCS provider supplies the file it points at: a random key generated for the first control-plane machine, then the running API server's own file for every later one. Do not add `/etc/kubernetes/encryption-provider.conf` to `KubeadmControlPlane.spec.kubeadmConfigSpec.files` — the provider drops any entry for that path. See [How the Encryption Provider Configuration Is Delivered](../create-cluster/huawei-dcs.mdx#encryption-provider-delivery).

  Remove the argument only if you deliberately want to run without at-rest encryption. For a DR pair the key must be identical on both sides, which means pinning it with `DCSCluster.spec.encryptionProviderConfigRef` as described in [Optional Disaster Recovery Deployment](#optional-disaster-recovery-deployment).
</Directive>
