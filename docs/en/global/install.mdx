...
Both commands must print `true split-local-direct-v1 <64-lowercase-hex-character-SHA-256>`. A bare `true`, a legacy profile, an empty or malformed config digest, a remaining migration bridge, or a failed Global-side probe means the handoff is not complete.
