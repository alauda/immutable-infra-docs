...
For a DR installation, the ConfigMap must also import the encryption-provider `Secret` referenced by `DCSCluster.spec.encryptionProviderConfigRef.name`. Export `DCS_ENCRYPTION_PROVIDER_SECRET` first, as described in [Prepare Shared DR Variables](#prepare-shared-dr-variables), and keep the second `names` entry below. For a normal non-DR installation that does not set `DCSCluster.spec.encryptionProviderConfigRef`, remove that entry.
  ...
  resources.yaml: |
    ...
    - resource: "secrets"
      names:
        ...
        # Required for DR when DCSCluster.spec.encryptionProviderConfigRef is set.
        # The name must match the Secret referenced by DCSCluster.
...
Create the DCS `dcs-import-extra-resources` ConfigMap from Step 7 on both installation environments. Set `PROVIDER_SECRET_NAME` to the same Secret name used by `DCSCluster.spec.credentialSecretRef.name`, and keep `DCS_ENCRYPTION_PROVIDER_SECRET` aligned with `DCSCluster.spec.encryptionProviderConfigRef.name`.
