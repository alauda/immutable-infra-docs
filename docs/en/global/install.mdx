...
The worked example above uses `type: external`. Before applying it, DCS Provider `v1.0.22+` with ACP `v4.4+` can replace resource 5's endpoint fields with this internal configuration. The VIP must be reserved in the control-plane Layer 2 network, and the `vrid` must be unique in that network:
