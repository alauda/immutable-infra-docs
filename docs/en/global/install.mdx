...
The manifest does not create the DCS API credential `Secret`. That Secret is created in the prerequisites above and referenced here by name through `DCSCluster.spec.credentialSecretRef`, which is also how Step 7 imports it into the `global` cluster.
...
1. **DCS API access, and the credential Secret created from it** — the endpoint (`https://<host>:7443`), an `administrator` user and password, and the site ID. The site ID fills `DCSCluster.spec.site`; the rest go into the credential Secret. Create it now, so the manifest can reference it by name:
   ...
   kubectl create secret generic "${PROVIDER_SECRET_NAME}" \
     --namespace cpaas-system \
   ...
   Write the values in plain text and do not quote them — `kubectl` base64-encodes them and takes quote characters literally. Do not create this Secret with `kubectl apply`: client-side apply stores the manifest it applied, password included, in the `kubectl.kubernetes.io/last-applied-configuration` annotation on the Secret. Add `userType=domain` to the file for a domain user. See [Cloud Credentials](../infrastructure/huawei-dcs.mdx#cloud-credentials).
