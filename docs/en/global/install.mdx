...
For DCS, create `dcs-import-extra-resources` when additional DCS resources must be preserved or imported into the installed `global` cluster. For HCS DR installations, this ConfigMap is required so that the installer imports HCS infrastructure resources during standby installation.
