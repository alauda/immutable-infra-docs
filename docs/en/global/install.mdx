...
| `console.host` | Local `global` HA VIP list | Local `global` control-plane VIP list | Local `global` control-plane VIP list | Local `global` control-plane VIP list |
...
Set `console.host` to the local `global` control plane VIP, the same address as `VSphereCluster.spec.controlPlaneEndpoint.host`. Do not use the platform domain in `console.host`; use `console.globalHost` for the platform access address.
...
Set `console.host` to the control plane VIP owned by the HCS ELB. Do not use the platform domain in `console.host`; use `console.globalHost` for the platform access address.
...
| VMware vSphere | Set `VSphereCluster.spec.controlPlaneEndpoint.host` and `console.host` to the primary HA VIP used by the primary manifest. Create the VMware vSphere `dcs-import-extra-resources` ConfigMap from Step 7 and keep `global-vsphere-credentials` aligned with `VSphereCluster.spec.identityRef.name`. | Set `VSphereCluster.spec.controlPlaneEndpoint.host` and `console.host` to the standby HA VIP used by the standby manifest. Create the VMware vSphere `dcs-import-extra-resources` ConfigMap from Step 7 and keep `global-vsphere-credentials` aligned with `VSphereCluster.spec.identityRef.name`. |
| Huawei Cloud Stack | Set `console.host` to the primary control plane VIP managed by the HCS ELB. Create the HCS `dcs-import-extra-resources` ConfigMap from Step 7 and keep `HCS_SECRET_NAME` aligned with `HCSCluster.spec.identityRef.name`. | Set `console.host` to the standby control plane VIP managed by the HCS ELB. Create the HCS `dcs-import-extra-resources` ConfigMap from Step 7 and keep `HCS_SECRET_NAME` aligned with `HCSCluster.spec.identityRef.name`. |
