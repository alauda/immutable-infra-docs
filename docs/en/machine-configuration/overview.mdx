...
Machine Configuration is delivered as a cluster plugin and is installed per cluster; see [Installation](./install.mdx). When the Machine Configuration Operator first starts on a cluster, it automatically creates two MachineConfigPools (control plane and worker) along with two empty MachineConfigs (00-master and 00-worker). The control plane pool manages the 00-master configuration, while the worker pool manages the 00-worker configuration.

You can create custom MachineConfigPools for worker nodes that require specialized configurations. Control plane nodes cannot use custom pools.
...
The operator also creates a default MachineConfiguration CR named "cluster" for setting global node update policies. See the Node Disruption Policy documentation for details.
...
For version history and changes, see [Release Notes](./release-note.md).
...
In this example, all three control plane nodes are current, while the worker pool is updating—two nodes are complete and one is in progress.
