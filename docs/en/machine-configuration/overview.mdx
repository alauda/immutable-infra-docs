...
During cluster installation, the system automatically creates two MachineConfigPools (control plane and worker) along with two empty MachineConfigs (00-master and 00-worker). The control plane pool manages the 00-master configuration, while the worker pool manages the 00-worker configuration.
