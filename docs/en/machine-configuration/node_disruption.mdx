...
The Machine Configuration Operator creates this resource when it first starts, with the default policy shown below.

:::warning
Because the default action for file changes is `None`, a `MachineConfig` object that only writes or updates a file is applied to disk but does **not** restart the affected service—so the change may not take effect. When a file change requires a service to reload or restart (for example, kubelet or firewalld configuration), define a node disruption policy for that file path, and confirm it appears in `status.nodeDisruptionPolicyStatus` **before** applying the `MachineConfig`.
:::
...
* `Reload`: Reload the specified `systemd` service without restarting it.
