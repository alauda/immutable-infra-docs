...
For a Huawei DCS cluster that uses Alauda OS, select the Alauda OS image published for the ACP release installed on the cluster. See [Downloading Alauda OS](../../overview/os-support-matrix.mdx#downloading-alauda-os).
...
:::info
**Control plane endpoint boundary for v1.0.22**

DCS Provider `v1.0.22` supports an external LoadBalancer and provider-managed `DCSCluster.spec.controlPlaneLoadBalancer.type: internal` Self-built VIP when paired with ACP `v4.4` or later. For `type: internal`, reserve an IPv4 VIP in the control-plane Layer 2 network and use the YAML workflow in [Creating Clusters on Huawei DCS](../../create-cluster/huawei-dcs.mdx#self-built-vip). DCS Provider `v1.0.21` and earlier support only the external LoadBalancer path.
:::
...
- **Self-built VIP**: Provider-managed control plane VIP through the `alive` runtime on DCS Provider `v1.0.22+` with ACP `v4.4+`
