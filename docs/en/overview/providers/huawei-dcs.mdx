...
:::info
**Control plane endpoint boundary for v1.0.21**

DCS Provider `v1.0.21` supports an external LoadBalancer for highly available control plane endpoints. It does not support `DCSCluster.spec.controlPlaneLoadBalancer.type: internal`. Plan the listener, backends, health check, and reachability in [Plan the Control Plane Endpoint](../../how-to/control-plane-endpoint.mdx).
:::
