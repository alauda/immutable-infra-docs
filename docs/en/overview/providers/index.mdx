...
|----------|------------------------|-----------|------------------|
| Huawei DCS | You provide it, or the provider manages a Self-built VIP | An external load balancer, provider-managed `type: internal` Self-built VIP on DCS Provider `v1.0.22+` with ACP `v4.4+`, or a single control plane node reached directly | For external mode, provision the load balancer before cluster creation. For internal mode, reserve an IPv4 VIP in the control-plane Layer 2 network and use the DCS YAML workflow |
...
| VMware vSphere | The provider creates it (or you provide it) | From provider `v1.0.17` with ACP `v4.4+`, `VSphereCluster.spec.controlPlaneLoadBalancer.type=internal` deploys Alive (`keepalived` + IPVS) onto the control-plane nodes; `type=external`, and an absent field, delegate the LB to an existing external appliance. The mode is fixed at cluster creation | For `internal`, choose a free IPv4 VIP, a port (typically `6443`), and a `vrid` unique in the control-plane Layer-2 broadcast domain. For `external`, provision the load balancer before cluster creation |
...
Huawei Cloud Stack is the only VM provider that creates a platform load balancer object for you. For Huawei DCS, the load balancer — including its VIP allocation and backend (real-server) maintenance — is prepared and owned outside the cluster. On Bare Metal and on VMware vSphere from provider `v1.0.17`, the provider can either deploy an in-cluster VIP component (`alive`) or defer to an external LB; see the [Bare Metal Provider](./bare-metal.mdx) and [VMware vSphere Provider](./vmware-vsphere.mdx) overviews for the full model.
...
- **vSphere Provider**: Provides YAML-based cluster lifecycle management on VMware vSphere. There is no Fleet Essentials UI extension for vSphere cluster creation and management
