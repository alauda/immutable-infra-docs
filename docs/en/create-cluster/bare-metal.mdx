...
The example below uses a minimal `KubeadmControlPlane`. For the full hardening profile used in QA — admission, audit, kubelet patches, encryption provider — see [Complete KubeadmControlPlane Configuration](#complete-kubeadmcontrolplane-configuration) in the Appendix.
:::
...
| `cpaas.io/registry-address` | Yes | Platform registry hostname (same value as `global.registry.address`). | Substituted into the catalog image at reprovision time. Missing annotation keeps `BaremetalMachine` in `ImageResolved=False / Reason=ImageRegistryMissing`. |
...
#### Single Control-Plane Layout \{#single-control-plane-layout}

For development or PoC clusters that run a single control-plane replica, set `KubeadmControlPlane.spec.replicas: 1`. Two fields still need a value:

- `BaremetalCluster.spec.controlPlaneLoadBalancer.host` — set to the IP of the sole control-plane host (the same IP the host carries on its main NIC). `vrid` and `port` are still required.
- `BaremetalCluster.spec.controlPlaneEndpoint.host` — leave empty; the reconciler backfills it.

`type=Internal` continues to work in this layout — `alive` will manage the VIP, which simply happens to coincide with the only control-plane node's IP. If you cannot afford `alive` static pods on a one-node control plane, use `type=External` and pre-create a TCP forwarder.
...
| `ImageResolved=False / Reason=ImageRegistryMissing` | `Cluster` is missing the `cpaas.io/registry-address` annotation. | Cluster annotations. |
...
The hardened configuration used by the bare-metal QA suite — admission control, audit policy, kubelet patches, encryption provider, and IPv6 bind addresses. Substitute the placeholders from the table in [Resolving Placeholder Values](#resolving-placeholders).
