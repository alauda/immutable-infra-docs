...
<Directive type="warning" title="Placement and stage both matter">
  The fragment must sit in `SeedImage.spec.cloud-config` under `stages.boot`. The two obvious alternatives both fail silently — the install succeeds, reports nothing unusual, and still leaves an 8 GiB partition:

  - `MachineRegistration.spec.config.cloud-config` is applied to the *installed* system, after the disk layout has been decided.
  - `stages.rootfs` never runs in a live ISO, because the ISO's GRUB entry carries `elemental.disable` and the rootfs stage unit depends on a mount that flag suppresses.
</Directive>
