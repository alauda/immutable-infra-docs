...
### 5. Workload Cluster Image Registry (Only When Installing Platform Components Later)

A registry credential is **not** required to create a bare-metal cluster. It only becomes necessary when later platform components on the new workload cluster need to pull from a credentialed registry. If your test scope ends at cluster + node Ready, you can ignore this prerequisite.

When you do install platform components, the cluster inherits the registry of the `global` cluster, backed by the `public-registry-credential` Secret when that registry is authenticated. To have this cluster pull platform images from a dedicated registry instead, decide before you create it and follow [Choose the Image Registry for a Workload Cluster](../how-to/workload-cluster-image-registry.mdx) — the binding is read only at cluster creation and cannot be changed afterwards.

This choice does not affect the OS images described above. Hosts pull `base-image` and `base-image-iso` from the platform registry during `elemental install` and every reprovision, before any cluster-level registry binding exists.
