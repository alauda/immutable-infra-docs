...
The `state` partition holds the running system plus every retained snapshot. The Elemental default is 8192 MiB, which is not enough for this platform: a base image occupies roughly 3 GiB, so a host that keeps the normal set of snapshots leaves under 3 GiB free, and the next `elemental upgrade` fails part-way through with `no space left on device`. Every reprovision and every cluster upgrade goes through `elemental upgrade`, so an under-sized `state` partition turns into a failed rollout later, on a cluster that is already in production.
...
<Directive type="warning" title="This setting only works in SeedImage.spec.cloud-config">
  Put the `partitions.yaml` fragment in `SeedImage.spec.cloud-config`, not in `MachineRegistration.spec.config.cloud-config`.

  `SeedImage.spec.cloud-config` is written into the ISO and executed by the live system during boot (the ISO's kernel command line carries `elemental.setup=/run/initramfs/live/iso-config`), so the file exists before `elemental install` starts and reads `/etc/elemental/config.d/`.

  `MachineRegistration.spec.config.cloud-config` is packaged as an OEM configuration for the *installed* system. It is applied after the disk layout has already been decided, so a `partitions.yaml` placed there silently has no effect and the host still gets an 8 GiB `state` partition.
</Directive>
