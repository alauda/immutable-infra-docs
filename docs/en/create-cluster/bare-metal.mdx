...
queries:
  ...
  - which ports must be open for a bare metal cluster
  - bare metal network requirements per stage
  - bare metal host time synchronization ntp requirement
---
...
The host firewall on an Alauda OS node is preconfigured and maintained by the platform — you do not manage it, and this table is not a node firewall rule set. See [Managing Firewall Ports](../machine-configuration/firewall.mdx). What you do have to plan is the network **between** the hosts and the endpoints they reach: site firewalls, security groups, and router ACLs between the host subnet, the platform, and the registry.

| Stage | Source | Destination | Protocol / port | Purpose |
| --- | --- | --- | --- | --- |
| First boot | Each host booted from the `SeedImage` ISO | Site DNS servers | UDP and TCP `53` | The host resolves the platform access address and the registry host by name before it can reach either |
| First boot | Each host | Site NTP servers | UDP `123` | The clock gate in [Host Time Synchronization](#host-time-synchronization) |
| First boot | Each host booted from the `SeedImage` ISO | Platform access address (`global.platformUrl`) | TCP `443`, or the custom platform HTTPS port | `elemental-register` registers the host, which creates its `MachineInventory` |
| First boot | Each host | Platform registry (`global.registry.address`) | TCP `11443` | `elemental install` pulls the Alauda OS image. Every later `elemental upgrade` reprovision pulls it again |
| Provisioning and steady state | Each workload-cluster host | Platform access address, path `/kubernetes/global` | TCP `443`, or the custom platform HTTPS port | `elemental-system-agent` polls its plan Secret. This is the platform ingress path, not a direct connection to a kube-apiserver |
| Cluster join | Every node of the new cluster | This cluster's control-plane endpoint | TCP `6443`, or the port set in `BaremetalCluster.spec.controlPlaneLoadBalancer.port` | `kubeadm join` and `kubelet` reach the API server |
| Cluster join | Every node of the new cluster | Platform registry resolved from the `cpaas.io/registry-address` annotation | TCP `11443` | The node pulls the platform images — CoreDNS, kube-proxy, and kube-ovn |
| Cluster management | `global` cluster nodes | This cluster's control-plane endpoint | TCP `6443`, or the configured control-plane port | The Cluster API controllers run on `global` and reconcile this cluster through its API server |
| Cluster runtime | Every node of the new cluster | Every other node of the same cluster | etcd `2379` and `2380` between control-plane nodes; the Kube-OVN overlay tunnel, UDP `6081` for the default Geneve encapsulation; `kubelet`; and Pod and Service traffic that is not filtered by port | Cluster-internal traffic. For the full component port inventory, see <ExternalSiteLink name="acp" href="/install/prepare/communication_matrix.html" children="Communication Matrix" />. Confirm the tunnel port against your own configuration if you changed the Kube-OVN tunnel type or attached a secondary network |
...
- **Bootstrap.** While `global` itself is still being installed, its hosts register against the temporary bootstrap endpoint and a different set of ports applies — see [Bootstrap host network](../global/install.mdx#bootstrap-host-network).
- **`global` hosts after handoff.** A `global` host's system agent uses the Global control-plane endpoint rather than the platform ingress path, and in a DR pair it uses the local control-plane VIP on `6443` directly. See [Endpoint and Identity](../global/disaster_recovery.mdx#endpoint-and-identity).
...
Whatever a host's network looks like when it registers is what the installed OS keeps: registration captures the host's NetworkManager connection files, and the install replays them on every later boot. Configure the network before the host installs, not after. A host that needs a bond, a second interface for the CNI, or a static address on a segment without DHCP has to be set up from the console first — including gating the automatic install, which otherwise starts as soon as the network comes up. See [Configure Host Networking on Bare-Metal Hosts](../how-to/configure-host-network-bare-metal.mdx).

For the simple case — one NIC, no DHCP on the segment — configure it manually from the host console before waiting for registration. Check the NetworkManager connection name first, then apply the site-specific address, gateway, and DNS values:
...
### 4. Host Time Synchronization \{#host-time-synchronization}

Every host that will become a cluster node — control plane and worker alike — must already carry the correct time when it boots the `SeedImage` ISO. The platform's cross-node requirement is a unified time zone on every cluster node and a time synchronization error between nodes of no more than 10 seconds; see <ExternalSiteLink name="acp" href="/install/prepare/node_preprocessing.html#node_checks" children="Node Checks" />. That requirement describes the cluster's nodes, so it applies here, but nothing on this path verifies it for you: the quick-configuration script and node checks on that page target traditional-operating-system machines that the platform joins over SSH, and they never run against an Alauda OS host installed by `elemental-operator`.

Treat time synchronization as a gate before [Step 1](#step-1-machineregistration-seedimage):

- Make the site NTP servers reachable from the host subnet, and open UDP `123` as listed in [Network Connectivity](#network-connectivity). A production cluster should not depend on public NTP.
- Set the clock **from firmware**, not from an operating system. At this point the install disk has been wiped as described in [Host Disk and Boot Preparation](#host-disk-and-boot-preparation), so there is no OS on the host to run a command in. Correct the time in the BIOS/UEFI setup or through the BMC, and keep the hardware clock on UTC so that the time zone is applied consistently once Alauda OS is installed.
...
By default the cluster uses the registry of the `global` cluster, backed by the `public-registry-credential` Secret when that registry is authenticated. To have it pull platform component images from a dedicated registry instead, follow [Choose the Image Registry for a Workload Cluster](../how-to/workload-cluster-image-registry.mdx).
...
### 7. Host Disk and Boot Preparation \{#host-disk-and-boot-preparation}
...
| `<kube-ovn-version>` | Kube-OVN chart version matching the selected ACP and Kubernetes release. | Use the **kube-ovn (chart)** value from the [OS Support Matrix](../overview/os-support-matrix.mdx), not the Kube-OVN component version, and confirm it against the installed plugin: `kubectl get moduleplugins.cluster.alauda.io kube-ovn -o jsonpath='{.spec.mainChart}{" "}{.status.latestVersion}'`. |
...
<Directive type="warning" title="Placement and stage both matter">
...
| `.spec.machineInventories[].networkDevice` | string | CNI interface for this host, overriding the cluster default. Use it when one host names its CNI interface differently — a bond, or a NIC that is not `eth1` on every machine. See [Choose the CNI Network Interface](#cni-network-interface). | No |
...
| `kube-ovn.cpaas.io/transmit-type` | Underlay only | `underlay`. Omit it, or set `overlay`, for the default. | Selects the Kube-OVN network type. The provider refuses to change it on a cluster whose CNI is already deployed. |
| `kube-ovn.cpaas.io/vlan-id` | Underlay | `0`–`4094` — the VLAN the CNI interface is trunked for. | `VLAN_ID` of the provider network. |
| `kube-ovn.cpaas.io/gateway` | Underlay | An IPv4 address inside `spec.clusterNetwork.pods.cidrBlocks[0]`. | Default gateway handed to Pods. |
| `kube-ovn.cpaas.io/exclude-ips` | Underlay, recommended | Comma-separated addresses or `a..b` ranges on the Pod subnet that are already in use. | Keeps Kube-OVN from allocating them. The control-plane VIP is excluded automatically. |
...
| `.spec.networkDevice` | string | Interface Kube-OVN uses on every node of the cluster. Defaults to `eth0`, the interface that also carries the node address. A pool member can override it per host. See [Choose the CNI Network Interface](#cni-network-interface). | No |
...
### Choose the CNI Network Interface \{#cni-network-interface}

By default Kube-OVN uses `eth0`, the same interface that carries the node address. Set `BaremetalCluster.spec.networkDevice` to give CNI traffic its own interface, and override it per host in the pool when a machine names that interface differently.
...
The address on that interface comes from the host, never from the provider — see [Configure Host Networking on Bare-Metal Hosts](../how-to/configure-host-network-bare-metal.mdx). In overlay mode the interface must hold an IPv4 address, because it is the Geneve tunnel source. In underlay mode Kube-OVN bridges the interface into `br-provider`, so it may stay address-less.

**Underlay.** Declare the physical network on the `Cluster`:

```yaml
metadata:
  annotations:
    kube-ovn.cpaas.io/transmit-type: underlay
    kube-ovn.cpaas.io/vlan-id: "100"
    kube-ovn.cpaas.io/gateway: 172.31.100.1
    kube-ovn.cpaas.io/exclude-ips: 172.31.100.2..172.31.100.20
```

`Cluster.spec.clusterNetwork.pods.cidrBlocks[0]` is then the physical subnet the Pods draw addresses from, `gateway` must be an address inside it, and `exclude-ips` must cover every address on that subnet that something else already uses. The provider adds the control-plane VIP to the exclusion list by itself.

Underlay also puts requirements on the physical network:

- The switch port passes the VLAN — trunked, or an access port matching `vlan-id` — with no MAC/IP binding or anti-spoofing applied to the Pod addresses.
- The Pod subnet's gateway shares a Layer 2 domain with the nodes.
- **The node network and the Pod subnet must reach each other in both directions.** kubelet probes, the API server calling a webhook, and metrics scraping all reach Pod addresses from the node, and Kube-OVN does not program a node route for an underlay subnet. Without that path Pods still get addresses and reach each other and the gateway, but every probed Pod — CoreDNS, the cert-manager webhook — restarts indefinitely.
- The CNI interface must differ from the interface that carries the node address and the default route. Kube-OVN would otherwise move that address, and the VIP, into `br-provider`; the provider rejects the configuration instead.
- A Pod subnet as small as a `/24` leaves the second node without a `Node.spec.podCIDR` (`CIDRNotAvailable` events), because kube-controller-manager still allocates a `/24` per node. Kube-OVN does not use that field, so the cluster works; silence the events by setting `--allocate-node-cidrs=false` or a matching `node-cidr-mask-size` in the `KubeadmControlPlane`.

**Per-host override.** Add the interface name to the host's entry in the pool:
...
The provider checks the resolved interface against the host's registered network before it writes a plan, and puts an override onto the Node as an annotation — `ovn.kubernetes.io/tunnel_interface` in overlay, `provider.provider-network.kubernetes.io/interface` in underlay. Read the outcome on the machine:

```bash
kubectl -n cpaas-system get baremetalmachines.infrastructure.cluster.x-k8s.io <machine> \
  -o jsonpath='{range .status.conditions[?(@.type=="NetworkDeviceReady")]}{.status} {.reason}: {.message}{"\n"}{end}'
```

| `NetworkDeviceReady` reason | Meaning |
|---|---|
| `NetworkDeviceValidated` | The host uses the cluster default; nothing is written on the Node |
| `NodeAnnotationApplied` | The host's override reached its Node |
| `NodeAnnotationPending` | The Node has not registered yet, or the workload cluster is unreachable; retried |
| `NetworkDeviceMissing` | No such interface on the host. No plan is written until the name is corrected |
| `NetworkDeviceIsSlave` | The interface is a bond slave — name the aggregate instead |
| `NetworkDeviceConflict` | Underlay only: the interface carries the node address and the default route |

`BaremetalCluster.status.conditions[NetworkConfigValid]` carries the cluster-level verdict: invalid annotation values, or a refused attempt to move an existing cluster between overlay and underlay.

:::warning
**Under underlay a per-host override is a one-shot input.** Kube-OVN folds the annotation into `ProviderNetwork.spec.customInterfaces` and then deletes it from the Node, and it reads the annotation only while that node is absent from `customInterfaces`. Changing `networkDevice` for a host afterwards has no effect — edit `ProviderNetwork` directly. Check the effective state with `kubectl get provider-networks -o yaml`, not with the Node annotation.
:::
