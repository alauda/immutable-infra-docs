...
| Provisioning and steady state | Each host | Platform access address, path `/kubernetes/global` | TCP `443`, or the custom platform HTTPS port | `elemental-system-agent` polls its plan Secret. This is the platform ingress path, not a direct connection to a kube-apiserver |
...
| Cluster runtime | Every node of the new cluster | Every other node of the same cluster | Component ports, plus Pod and Service traffic that is not filtered by port | etcd (`2379`, `2380`) between control-plane nodes, `kubelet`, and the CNI. For the component port inventory, see <ExternalSiteLink name="acp" href="/install/prepare/communication_matrix.html" children="Communication Matrix" /> |

:::warning
**`443` alone is not enough for a workload host**

`elemental-system-agent` reaches the platform through `https://<platform-domain>/kubernetes/global`, so registration and plan polling need only the platform HTTPS port. That is not the whole requirement. The same host still pulls its operating system and platform images from the registry on `11443`, joins the API server on the control-plane endpoint port, and exchanges cluster traffic with the other nodes. A firewall approval that covers only `443` produces hosts that register successfully and then fail during install or join, where the cause is much harder to see.
:::

**Bootstrap phase.** The endpoints above apply once a `global` cluster exists. While `global` itself is still being installed, its hosts register against the temporary bootstrap endpoint instead, and a different set of ports applies — see [Bootstrap host network](../global/install.mdx#bootstrap-host-network). A `SeedImage` built during that phase points at the bootstrap endpoint and must not be reused after handoff; create new `MachineRegistration` and `SeedImage` objects on the active `global` cluster. A workload `SeedImage` stays valid across a handoff only because its registration URL uses the platform domain rather than a bootstrap address.
...
Each host's clock must already be correct when it boots the `SeedImage` ISO. The platform's cross-node requirement is a unified time zone on every cluster node and a time synchronization error between nodes of no more than 10 seconds; see <ExternalSiteLink name="acp" href="/install/prepare/node_preprocessing.html#node_checks" children="Node Checks" />. That requirement describes the cluster's nodes, so it applies here, but nothing on this path verifies it for you: the quick-configuration script and node checks on that page target traditional-operating-system machines that the platform joins over SSH, and they never run against an Alauda OS host installed by `elemental-operator`.

Treat time synchronization as a gate before [Step 1](#step-1-machineregistration-seedimage):

- Make the site NTP servers reachable from the host subnet and have machine operations synchronize each host against them before the ISO boot. A production cluster should not depend on public NTP.
- Verify each host instead of assuming that the live ISO corrects the clock. Run `date -u` from the host console and compare the result with the site reference clock. Across the hosts that will form the same control plane, the spread must stay within the 10-second requirement.
- After the nodes reach `Ready`, confirm that an ongoing time source is in use — `chronyc tracking` or `timedatectl` on the node.

The control-plane hosts generate the cluster PKI and form etcd during the same provisioning pass, which is why the check belongs here rather than after the cluster exists. `kubeadm` issues the cluster certificates using the clock of the host it runs on: if that clock is ahead of the others, the certificates carry a `notBefore` timestamp that the remaining nodes still treat as being in the future and reject, and if it is behind, the certificates expire earlier than expected. etcd members also depend on agreeing about time for leases and elections. Both failures surface at control-plane bring-up, long after the registration step where the wrong clock was introduced.

:::warning
**A Day-2 chrony `MachineConfig` does not replace this**

[Configuring the Chrony Time Service](../machine-configuration/managing.mdx) writes `/etc/chrony.conf` to nodes that are already running and joined. It is the correct way to point a cluster at the site NTP servers for ongoing operation, and it is a different requirement from this one: it cannot correct the clock of a host that is still installing Alauda OS or running `kubeadm`. Deferring to it leaves the install and join window unprotected.
:::
...
### 6. Public Registry Credential (Only When Installing Platform Components Later)
