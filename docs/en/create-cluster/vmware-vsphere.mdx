...
Create the vCenter credential secret referenced by `VSphereCluster.spec.identityRef`. Write the values into a file with restricted permissions, create the Secret from that file, then remove the file:

```bash
umask 077
cat > vsphere-credentials.env <<'EOF'
...
Each line of the file is `key=value`. Do not quote the values — `kubectl` takes quote characters literally. A value may contain `=`; only the first `=` on a line separates the key from the value.
...
Create the CPI credential Secret. It wraps the `vsphere-cloud-secret` that the vSphere CPI reads on the workload cluster, so its payload carries the vCenter password. Write the payload into a file with restricted permissions, create the Secret from that file, then remove the file:

```bash
umask 077
cat > vsphere-cpi-secret-payload.yaml <<'EOF'
apiVersion: v1
kind: Secret
metadata:
...
kubectl create secret generic <cluster_name>-vsphere-cpi-secret \
  ...
  --from-file=data=vsphere-cpi-secret-payload.yaml

rm -f vsphere-cpi-secret-payload.yaml
```

<Directive type="warning" title="Do not create this Secret with kubectl apply">
  Client-side apply stores the manifest it applied — here, the payload with the vCenter password — in the `kubectl.kubernetes.io/last-applied-configuration` annotation on the Secret itself. `kubectl create` writes no such annotation. Replace the placeholders in the payload before running the command; the surrounding quotes on the heredoc keep the shell from expanding them.
</Directive>
