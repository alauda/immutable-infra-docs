...
- **Control plane nodes** (3 disks per node): `var-cpaas` in `persistentDisks[]`, plus `var-lib-containerd` and `var-lib-etcd` in `ephemeralDisks[]`. Do not remove any of these disks.
- **Worker nodes** (2 disks per node): `var-cpaas` in `persistentDisks[]`, plus `var-lib-containerd` in `ephemeralDisks[]`. Do not remove any of these disks.
...
Use `ephemeralDisks` for rebuildable node-local data that should not sit on the system disk — the standard `/var/lib/containerd` and `/var/lib/etcd` disks, plus any scratch space. Use `persistentDisks` for anything whose loss is a data loss.

<Directive type="warning" title="Keep the standard disks in their declared list">
`/var/cpaas` must stay in `persistentDisks[]`: it holds platform state that has to survive VM replacement. `/var/lib/containerd` and `/var/lib/etcd` belong in `ephemeralDisks[]`; they are rebuilt on every new VM, and declaring them as persistent disks carries stale runtime and etcd member data onto the replacement node.

This split is a creation-time choice. `ephemeralDisks[]` is new in provider `v1.0.17`, so a cluster created on an earlier provider declares all of them as persistent disks, with `wipeFilesystem: true` on the etcd disk. That cluster keeps its layout: a provider upgrade migrates no disk, and an allocated slot rejects the removal or the reshaping of a disk in either list. See [Upgrading the Provider](../install/vmware-vsphere.mdx#upgrade-the-provider).
</Directive>
...
Ephemeral disks have no `unitNumber` field at all. The controller assigns their unit at every clone and reports it in `status.ephemeralDiskStatuses[].unitNumber`, so the standard `/var/lib/containerd` and `/var/lib/etcd` disks cannot be pinned.
