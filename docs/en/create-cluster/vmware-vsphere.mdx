...
### Choose the endpoint mode \{#choose-the-endpoint-mode}
...
The `vrid` range is checked by the provider's admission webhook, not by the CRD schema, so `kubectl apply --dry-run=client` does not catch an out-of-range value.
</Directive>
...
- **Set `dns.imageTag` yourself and leave `dns.imageRepository` unset.** The provider derives the repository from the `-v<acp>` suffix in the CoreDNS tag: `acp` at ACP `v4.4` and later, `tkestack` below it.
...
| `is immutable for allocated slot` | The slot is `InUse` or `Released`. See [Allocated slots are immutable](#create-the-machine-config-pools). |
