...
| `osImageVersion` | The exact name of the Alauda OS image uploaded to the HCS platform. The provider uses it to locate the ECS image, and it becomes the node pool default image. For a standard release it matches the name the Alauda OS image published for that ACP release was uploaded under. See [Downloading Alauda OS](../overview/os-support-matrix.mdx#downloading-alauda-os). |
...
### 4. Workload Cluster Image Registry
...
|---|---|---|
| Registry of the `global` cluster | Nothing. The cluster inherits it, and the wizard fills in the address for you. | Workload nodes reach the registry the `global` cluster uses with acceptable throughput. |
...
:::warning
A dedicated registry is configured through YAML only. The web UI wizard always uses the registry of the `global` cluster: it reads that address from the `global` cluster and writes it into the new cluster for you, and it does not offer a registry credential to select. Use [Using YAML](#using-yaml) when this cluster needs its own registry.

The binding is also read only when the cluster is first created. Changing or removing `cpaas.io/registry-reference` on an existing `Cluster` does not move that cluster to another registry.
:::
...
HCS authentication information is stored in a Secret resource in `cpaas-system`.

Author the credential file with an editor rather than typing the values at a shell prompt. A shell records the whole command, secret key included, in its history; a file written by an editor leaves no such copy.

```text title="hcs-credential.env"
accessKey=<access-key>
secretKey=<secret-key>
projectID=<project-id>
region=<region>
externalGlobalDomain=<domain>
schema=https
```
...
```bash
chmod 600 hcs-credential.env

HCS_SECRET_NAME="<credential-secret-name>"

kubectl create secret generic "${HCS_SECRET_NAME}" \
  --namespace cpaas-system \
  --from-env-file=hcs-credential.env

rm -f hcs-credential.env
```

Write the values in plain text — `kubectl` base64-encodes them when it creates the Secret, so there is no separate encoding step. Each line is `key=value`. Do not quote the values: `kubectl` takes quote characters literally. A value may contain `=`; only the first `=` on a line separates the key from the value.

<Directive type="warning" title="Do not create credential Secrets with kubectl apply">
  Client-side apply stores the manifest it applied — including the HCS secret key — in the `kubectl.kubernetes.io/last-applied-configuration` annotation on the Secret itself. `kubectl create` writes no such annotation. This applies whether the values were written as `stringData` or as base64 `data`; base64 is an encoding, not protection.
</Directive>

| Key | Description |
|-----------|-------------|
| `accessKey` | HCS access key ID from **My Settings** > **Access Keys** |
| `secretKey` | HCS secret access key from **My Settings** > **Access Keys** |
| `projectID` | Resource Space ID from **My Settings** > **Resource Spaces** |
| `region` | HCS region API value used by the provider. Tenant administrators cannot retrieve this value from the HCS UI; get it from the HCS administrator |
| `externalGlobalDomain` | HCS platform access domain used by the provider |
| `schema` | Optional API scheme for the HCS IAM endpoint, usually `https`. Omit the line to use the default |
...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `.spec.template.spec.imageName` | string | Yes | HCS image name. Use the name the Alauda OS image published for the target ACP release was uploaded under; placeholder shown as `<alaudaos-version>` in the manifest above. The exact name moves with each Alauda OS release, so do not hardcode the example value across versions. |
