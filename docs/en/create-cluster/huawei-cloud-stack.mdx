...
- Control plane ELB address planning, including `vipAddress`, `vipSubnetName`, and optional fixed L4 and L7 IPs
...
| `.spec.network.subnets[].name` | string | Yes | Complete subnet inventory used by this cluster. Every subnet referenced elsewhere in the cluster manifests must appear in this list |
...
| `.spec.controlPlaneLoadBalancer.elbVirsubnetL4Ips[]` | array | No* | L4 virtual subnet IP groups used by Hybrid Load Balancing |
...
| `.spec.controlPlaneLoadBalancer.elbVirsubnetL7Ips[]` | array | No* | L7 virtual subnet IP groups used by Hybrid Load Balancing |
