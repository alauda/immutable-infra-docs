...
| `<cluster-name>` | Operator-chosen; must satisfy DNS-1123. Reused across `Cluster`, `DCSCluster`, and the `cluster.x-k8s.io/cluster-name` label on every `Machine`. The `KubeadmControlPlane` uses the prefixed form `<cluster-name>-kcp`. | n/a |
