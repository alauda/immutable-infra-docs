...
Configure a stable Kubernetes API endpoint before creating the cluster. For DCS Provider `v1.0.21`, highly available clusters require an external LoadBalancer that you provision and maintain outside the cluster.

| Mode | Version | Who manages it | When to use |
|---|---|---|---|
| External LoadBalancer | All documented DCS provider versions, including `v1.0.21` | You provision and maintain the listener and control-plane backends before cluster creation. | Use this for production and other highly available clusters. |
...
Follow [Plan the Control Plane Endpoint](../how-to/control-plane-endpoint.mdx) for the external LoadBalancer contract. DCS Provider `v1.0.21` does not support `type: internal`; do not use ACP version alone to infer provider capability. For a single-control-plane deployment that has no load balancer in front of the API server, see [Single-Control-Plane (No External LB) Layout](#single-control-plane-layout).
...
| Cluster API Address | text | Yes | Format: `https://<load-balancer-address>:6443`. Provision and validate the external LoadBalancer before cluster creation. |
...
- The Kubernetes API endpoint and external LoadBalancer are ready before cluster creation.
...
Before creating control plane resources, plan the network architecture and external control plane endpoint.
...
- **API endpoint mode**: Use an external LoadBalancer for a highly available DCS cluster
- **API server address**: Prepare a stable load balancer VIP or FQDN for the Kubernetes API server
- **Connectivity**: Ensure network connectivity between all components

Validate the Layer 4 listener, TCP `6443` backends, HTTPS `/healthz` checks, backend ownership, and endpoint reachability in [Plan the Control Plane Endpoint](../how-to/control-plane-endpoint.mdx).
...
Use `type: external` for the external LoadBalancer. This is the supported mode for DCS Provider `v1.0.21` and all earlier documented releases.
...
| `.spec.controlPlaneLoadBalancer.type` | string | Set to `external`. DCS Provider `v1.0.21` does not support `internal`. | Yes |
| `.spec.controlPlaneLoadBalancer.host` | string | External load balancer IP address or domain name. | Yes |
...
This layout has no HA — losing the single control plane node makes the cluster API unreachable until the node is recovered. For production, use `replicas: 3` with an external LoadBalancer.
