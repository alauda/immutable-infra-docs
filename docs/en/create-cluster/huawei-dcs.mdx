...
This document provides comprehensive instructions for creating Kubernetes clusters on the Huawei DCS platform using Cluster API. The process involves deploying and configuring multiple Kubernetes resources that work together to provision and manage cluster infrastructure.
...
:::info
Before configuring this Secret, ensure that your DCS platform credentials meet the requirements specified in [Prerequisites → DCS Platform Credential Requirements](#4-dcs-platform-credentials).
:::
...
The example below truncates long configuration files for readability. For the **complete configuration** (including default audit policies, admission controls, and file contents), please refer to the [Complete KubeadmControlPlane Configuration](#complete-kubeadmcontrolplane-configuration) in the Appendix.
:::

In the following example, `<cluster-name>` is the resource name:

For component versions (e.g., `<dns-image-tag>`, `<etcd-image-tag>`), please refer to [OS Support Matrix](../overview/os-support-matrix.mdx).
...
| .spec.machineTemplate.infrastructureRef | object | DCSMachineTemplate name for creating DCSMachine resources | Yes |
...
Refer to the [Deploy Nodes](../manage-nodes/huawei-dcs.mdx) page for instructions.
...
Below is the complete `KubeadmControlPlane` configuration, including all default audit policies, admission controls, and file contents. You can copy this entire block and modify the `<placeholders>` as needed.
