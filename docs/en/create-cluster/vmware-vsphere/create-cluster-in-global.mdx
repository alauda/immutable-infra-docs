...
- You completed the values collection in [Preparing Parameters for a VMware vSphere Cluster](./parameter-checklist.mdx).
- The `global` cluster can reach vCenter.
- The target template, networks, datastores, and vCenter resource pool are available.
- The control plane VIP and load balancer are ready.
- The platform can process the cluster annotations required to install the network plugin.

Also confirm the following technical prerequisites:

1. The management cluster can reach vCenter.
2. The template, networks, datastore, and vCenter resource pool are available.
3. The control plane VIP and load balancer are preconfigured.
4. All required static IP addresses are allocated and not in use.
5. `ClusterResourceSet=true` is enabled.
6. The platform already has a valid public registry configuration.
...
kubectl -n <namespace> get deploy capi-controller-manager -o jsonpath='{.spec.template.spec.containers[0].args}'
kubectl -n cpaas-system get secret public-registry-credential
```
...
- The public registry credential is present.
      ...
      content: |
        ...
        - resources:
          ...
          - aescbc:
              keys:
              - name: key1
                secret: O/VjJyoIg8iYppjoVUXiCm+nsWoGzNxxbXlByPpgy4k=
...
- The worker manifest includes a dedicated `persistentDisks` block so the sample can be used directly in the baseline environment. If the worker nodes do not need data disks, remove that block exactly as described in [Extension Scenarios](./extension-scenarios.mdx).
