...
kubectl -n <namespace> get secret public-registry-credential
```
...
kubectl -n <namespace> get cluster,vspherecluster,kubeadmcontrolplane,machinedeployment,machine,vspheremachine,vspherevm
kubectl -n <namespace> logs deploy/capi-controller-manager
```
...
- `deploy/capi-controller-manager` runs in the `<namespace>` namespace of the `global` cluster.
