...
<Directive type="info">
`deviceName` is optional. If you do not need to force the guest NIC name, remove the `deviceName` line from every node slot. CAPV will assign NIC names such as `eth0`, `eth1` by NIC order.
</Directive>
