...
| Workload object namespace | `<namespace>` | Yes | `cpaas-system` is commonly used. | `cpaas-system` | - |
...
| vCenter resource pool path | `<resource_pool_path_1>` | Yes | Must exist in vCenter inventory. | `/dc-a/host/compute-a/Resources` | - |
| Default datastore | `<default_datastore_1>` | Yes | Used as the default datastore for the baseline topology. | `datastore-a` | - |
...
| Guest NIC name | `<nic1_device_name>` | No | Set it only when you need to force the guest NIC name. | `eth0` | - |
