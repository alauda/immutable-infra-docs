...
This document helps you collect the values required to create a VMware vSphere workload cluster from the `global` cluster. Complete this checklist before you apply the manifests in [Create a Cluster in the `global` Cluster](./create-cluster-in-global.mdx). The provider behavior and fields on this page are validated against VMware vSphere Provider `v1.0.16`.
