...
The umbrella chart installs both the bare-metal provider manager and `elemental-operator` in one step. It also creates the `elemental-image-catalog` ConfigMap, but with no entries, and the package contains no Alauda OS image. Before you create a cluster, import the Bare Metal OS images into the platform registry and add every Kubernetes version you deploy to the catalog; see [OS Images Imported and Image Catalog Populated](../create-cluster/bare-metal.mdx#os-images-and-image-catalog).
...
Right after installation `elemental-image-catalog` has no `data` entries. After you add the Kubernetes versions you intend to deploy, confirm them:

```bash
kubectl -n cpaas-system get configmap elemental-image-catalog -o yaml
```

Every `Machine.spec.version` used by a cluster — both at creation time and at upgrade time — must appear as a key in this ConfigMap. A missing key leaves the `BaremetalMachine` in `Failed` with `Reason=ImageCatalogMiss`.
